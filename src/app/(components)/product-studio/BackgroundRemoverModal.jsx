...
const IMAGE_BACKGROUNDS = [
  ...
  {
    id: "surf_marble_close",
    name: "Marble Close-up",
    category: "Surface",
    url: pxbg(4705932),
  },
  {
...
const TEMPLATES = [
  {
    ...
    scale: "fit",
    preview:
      "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=200&q=70",
  },
  {
    id: "clean_grey",
    name: "Soft Grey",
  ...
  },
  {
    ...
    category: "Studio",
    bg: "#f5e6d3",
    padding: { top: 8, bottom: 18, left: 8, right: 8 },
    align: "bottom-center",
    scale: "fit",
    preview:
  ...
  },
  {
    ...
    scale: "fit",
    preview:
      "https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=200&q=70",
  },
  {
    id: "shopify_off",
    name: "Off White",
  ...
  },
  {
    ...
    scale: "fit",
    preview:
      "https://images.unsplash.com/photo-1559629819-638a8f0a4303?w=200&q=70",
  },
  {
    id: "classic_black",
    name: "Black",
  ...
  },
  {
    ...
    bg: "linear-gradient(135deg,#f093fb,#f5a623)",
    padding: { top: 5, bottom: 5, left: 5, right: 5 },
    align: "center",
    scale: "fill",
    preview:
      "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200&q=70",
  },
  {
    id: "profile_ocean",
    name: "Ocean",
    category: "Profile Pics",
    bg: "linear-gradient(135deg,#0099f7,#f11712)",
    padding: { top: 5, bottom: 5, left: 5, right: 5 },
    align: "center",
    scale: "fill",
    preview:
  ...
  },
  {
    ...
    scale: "fit",
    preview:
      "https://images.pexels.com/photos/3847496/pexels-photo-3847496.jpeg?auto=compress&cs=tinysrgb&w=200",
  },
  {
    id: "wood_natural",
    name: "Natural Wood",
    ...
    scale: "fit",
    preview:
      "https://images.unsplash.com/photo-1542621334-a254cf47733d?w=200&q=70",
  },
  {
    id: "minimal_cream",
    name: "Cream",
    ...
    scale: "fit",
    preview:
      "https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?w=200&q=70",
  },
  {
    id: "minimal_rose",
    name: "Rose",
...
function Toggle({ enabled, onChange }) {
  return (
    <button
      ...
      className={`relative w-10 h-6 rounded-full transition-all shrink-0 cursor-pointer ${enabled ? "bg-blue-500" : "bg-gray-300 hover:bg-gray-400"}`}
    >
      <span
        className={`absolute top-1 w-4 h-4 bg-surface rounded-full shadow transition-all ${enabled ? "left-5" : "left-1"}`}
      />
    </button>
  );
}
...
const scaleShadow = (filterStr, factor) => {
  ...
  const lenPart = colorMatch
    ? inner.slice(0, colorMatch.index).trim()
    : inner.trim();
  const scaled = lenPart
    .split(/\s+/)
    .map((t) => {
...
const downscaleFile = (file, maxDim = 2500) =>
  new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        resolve(
          blob
            ? new File([blob], file.name || "image.png", { type: "image/png" })
            : file,
        );
      }, "image/png");
    };
...
export default function BackgroundRemoverModal({
  ...
  const onResizeMouseDown = useCallback(
    (e, handleId) => {
      ...
      const onMove = (me) => {
        const { handle, startX, startY, startW, startH, startPosX, startPosY } =
          resizeRef.current;
    ...
    await Promise.all(
      items.map(async (it, i) => {
        ...
        try {
          ...
          setBatchItems((prev) =>
            prev.map((p, idx) =>
              idx === i ? { ...p, removedUrl: url, status: "done" } : p,
            ),
          );
  ...
  const applySceneBackground = (bgItem) => {
    ...
    if (!removedUrl)
      toast("Tip: remove the product background so the scene shows through.");
  };

  const generateAiBg = async () => {
    if (!aiPrompt.trim() || !removedUrl) {
      toast.error("Add a description and remove the background first");
      return;
    }
  ...
  const composeCanvas = async (
    ...
    if (showImageBg) {
      ...
      if (selectedColor.startsWith("linear-gradient")) {
        ...
        stops.forEach((c, i) =>
          grad.addColorStop(stops.length > 1 ? i / (stops.length - 1) : 0, c),
        );
        ctx.fillStyle = grad;
      } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const dataUrl = await composeCanvas(productUrl, {
        autoFit: false,
        withShadow: !aiResultUrl,
      });
      triggerDownload(
        dataUrl,
        batchMode
      ...
      toast.error(
        "Export failed. The background image may not be cross-origin accessible.",
      );
    }
  };
  ...
  const handleDownloadAll = async () => {
    const done = batchItems.filter(
      (it) => it.status === "done" && it.removedUrl,
    );
    if (!done.length) {
      toast.error("No processed images to download yet.");
      return;
    }
    ...
    for (const it of done) {
      try {
        const dataUrl = await composeCanvas(it.removedUrl, {
          autoFit: true,
          withShadow: true,
        });
  ...
  const getCanvasBg = () => {
    ...
    if (bgTab === "image" && bgImageUrl && !aiResultUrl)
      return {
    ...
    >
      ...
      >
        ...
        <div className="w-full flex flex-row items-center overflow-x-auto hide-scrollbar border-t border-gray-200 bg-surface shrink-0 pb-(--ck-safe-b) lg:w-16 lg:flex-col lg:overflow-x-visible lg:overflow-y-auto lg:border-t-0 lg:border-r lg:py-2 lg:pb-2 lg:max-h-none">
          ...
          <SidebarIcon
            ...
            icon={
              ...
              >
                <rect x="3" y="3" width="10" height="10" rx="1" opacity="0.5" />
        ...
        <div className="flex-1 w-full min-w-0 max-h-[42dvh] border-t border-gray-200 flex flex-col bg-surface overflow-hidden lg:w-64 lg:flex-none lg:max-h-none lg:border-t-0 lg:border-r">
              ...
              <div className="px-3 py-2 border-b border-gray-200 shrink-0">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-gray-200 outline-none bg-gray-100 cursor-text focus:border-blue-400 transition-colors"
                  />
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {grouped[cat].slice(0, 4).map((tpl) => (
                        <button
                          ...
                          style={{
                            background: tpl.bg?.startsWith("linear")
                              ? tpl.bg
                              : tpl.bg === "transparent"
                                ? undefined
                                : tpl.bg,
                        ...
                        >
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/40 to-transparent px-1 py-1">
                ...
                {RESIZE_STANDARD.map((s) => (
                  ...
                  >
                    <div className="w-8 h-8 flex items-center justify-center shrink-0">
                      <div
                        className={`border-2 rounded-sm transition-colors ${selectedResize === s.id ? "border-blue-400" : "border-gray-400"} ${s.w > s.h ? "w-6 h-4" : s.h > s.w ? "w-4 h-6" : "w-5 h-5"}`}
                      />
                    </div>
                    <div className="text-left">
                      ...
                      <p className="text-[10px] text-gray-500">{s.dims}</p>
                    </div>
                  </button>
                ))}
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mt-3 mb-2">
                  Advanced
                </p>
                {[
                  {
                    id: "custom",
                    name: "Custom size",
                    sub: "Enter exact dimensions",
                  },
                  {
                    id: "original",
                    name: "Original image",
                    sub: "Keep source dimensions",
                  },
                ].map((s) => (
                  <button
                    key={s.id}
                    onClick={() => {
                      setSelectedResize(s.id);
                      if (s.id === "original") setCanvasRatio({ w: 1, h: 1 });
                    }}
                    className={`w-full flex items-center gap-3 px-2 py-2.5 rounded-lg mb-0.5 transition-colors cursor-pointer ${selectedResize === s.id ? "bg-blue-50 border border-blue-200" : "hover:bg-gray-100 border border-transparent hover:border-gray-200"}`}
                  >
                    <div className="w-8 h-8 flex items-center justify-center shrink-0">
                      <div
                        className={`w-5 h-5 border-2 border-dashed rounded-sm ${selectedResize === s.id ? "border-blue-400" : "border-gray-400"}`}
                      />
                    </div>
                    <div className="text-left">
                      <p className="text-xs font-medium text-gray-900">
          ...
          {activePanel === "background" && (
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 shrink-0">
                ...
                <Toggle
                  enabled={removeOrigBg}
                  onChange={(v) => {
                    setRemoveOrigBg(v);
                    if (v && originalFile) doRemoveBg(originalFile);
                  }}
                />
              </div>
                ...
                {bgTab === "color" && (
                  <>
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      <label
                        className="w-8 h-8 rounded-full cursor-pointer border-2 border-gray-200 overflow-hidden flex items-center justify-center hover:border-blue-400 transition-colors"
                        style={{
                          background:
                ...
                {bgTab === "image" && (
                  <>
                    ...
                    {IMAGE_BG_CATEGORIES.map((cat) => {
                      const bgs = IMAGE_BACKGROUNDS.filter(
                        (b) => b.category === cat,
                      );
                      return (
                        <div key={cat} className="mb-4">
                          ...
                          <div className="grid grid-cols-2 gap-1.5">
                            {bgs.map((bg) => (
                              <button
                                key={bg.id}
                                onClick={() => applySceneBackground(bg)}
                ...
                {bgTab === "ai" && (
                  <div>
                    <p className="text-xs text-gray-500 mb-3">
                      Describe any background and AI will generate it
                    </p>
                    <textarea
                      value={aiPrompt}
                    ...
                    <button
                      ...
                      disabled={applyingAiBg || !aiPrompt.trim()}
                    ...
                    <p className="text-[9px] text-gray-500 mt-2 text-center">
                      Uses AI image generation credits
                    </p>
                  </div>
                )}
              </div>
          ...
          {activePanel === "shadows" && (
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto px-3 py-3">
                <p className="text-xs text-gray-500 mb-3">
                  Choose a shadow style for your product
                </p>
                <div className="grid grid-cols-2 gap-2">
                  {SHADOW_PRESETS.map((s) => (
                    <button
                      key={s.id}
                      onClick={() => setSelectedShadow(s.id)}
                    ...
                    >
                      <div className="w-14 h-14 bg-gray-100 rounded-xl flex items-center justify-center">
                        <div
                          className="w-8 h-8 bg-surface border border-gray-200 rounded-lg"
                          style={s.style}
                        />
                      </div>
        ...
        >
          ...
          >
            {removing ? (
              ...
              >
                ...
                <p className="text-gray-500 font-medium group-hover:text-gray-900 transition-colors">
          ...
          {/* Batch filmstrip */}
          {batchMode && (
            <div className="shrink-0 flex items-center gap-2 px-4 py-2.5 bg-surface/80 backdrop-blur-sm border-t border-gray-200 overflow-x-auto hide-scrollbar">
              {batchItems.map((it, i) => (
                <button
                  key={i}
                  onClick={() => selectBatchItem(i)}
                  className={`relative shrink-0 w-14 h-14 rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${i === activeIndex ? "border-blue-500 ring-2 ring-blue-200" : "border-gray-200 hover:border-gray-200"}`}
                  title={it.name}
                  style={it.removedUrl ? previewBgStyle : checkerBg}
                >
                  <img
                    src={it.removedUrl || it.originalUrl}
                    alt={it.name}
            ...
            <div className="flex items-center gap-2">
              {batchMode ? (
                <button
                  ...
                  disabled={
                    downloadingAll ||
                    !batchItems.some((it) => it.status === "done")
                  }
                ...
                >
                  {downloadingAll ? (
                    ...
                    <>
                      ...
                      {batchItems.filter((it) => it.status === "done").length})
                    </>
                  )}
                </button>
              ) : (
