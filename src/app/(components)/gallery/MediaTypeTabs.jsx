...
export default function MediaTypeTabs({
  ...
  if (variant === "pill") {
    // One scrolling line rather than wrapping: in a narrow container (the
    // editor's 300px Uploads panel) wrapping stacked these into two rows. Wide
    // containers fit all four anyway, so they look unchanged.
    return (
        ...
        {tabs.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => onChange(id)}
              className={`flex shrink-0 items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors cursor-pointer border ${
                isActive
