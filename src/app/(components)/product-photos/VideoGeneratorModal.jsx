...
import { X, Upload, Loader2, ChevronDown, ChevronLeft, ChevronRight, Check, User, Package, Image as ImageIcon, Scissors, Layers, Shirt, Sparkles, LayoutGrid, Video } from 'lucide-react';
...
const TEMPLATE_CATEGORIES = ['All', 'Dresses', 'Tops', 'Bottoms', 'Outerwear', 'Accessories', 'Footwear', 'Bags', 'Beauty', 'Food & Drink', 'Furniture'];

const tpl = (id, category) => ({ id: `${category}-${id}`, category, img: px(id) });
const TEMPLATES = [
    tpl(6780091, 'Dresses'), tpl(5112737, 'Dresses'),
    tpl(6780038, 'Tops'), tpl(37741914, 'Tops'),
    tpl(6780036, 'Bottoms'), tpl(18516993, 'Bottoms'),
    tpl(37218533, 'Outerwear'), tpl(5421296, 'Outerwear'),
    tpl(7895502, 'Accessories'), tpl(7866490, 'Accessories'),
    tpl(27046150, 'Footwear'), tpl(9267592, 'Footwear'),
    tpl(17938771, 'Bags'), tpl(36385199, 'Bags'),
    tpl(8015790, 'Beauty'), tpl(8049841, 'Beauty'),
    tpl(4869290, 'Food & Drink'), tpl(12900864, 'Food & Drink'),
    tpl(35392792, 'Furniture'), tpl(6053887, 'Furniture'),
];
...
export default function VideoGeneratorModal({ onClose, onSwitchTool }) {
    ...
    const closeAll = () => { setOpenDropdown(null); setToolMenuOpen(false); };
                ...
                <div className="w-84 border-r border-gray-100 flex flex-col flex-shrink-0">
                        ...
                        <div className="px-4 pt-5">
                            ...
                            <TemplateRow>
                                ...
                                <button
                                    ...
                                    className={`flex-shrink-0 w-24 h-32 rounded-xl overflow-hidden relative flex items-center justify-center text-white text-xs font-medium bg-gradient-to-br from-gray-700 to-gray-900 border-2 transition-colors ${selectedTemplate === 'none' ? 'border-violet-500' : 'border-transparent'}`}
                                >
                                    No template
                                    {selectedTemplate === 'none' && <span className="absolute top-1.5 right-1.5 w-5 h-5 bg-violet-600 rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></span>}
                                </button>
                                {TEMPLATES.map(t => (
                                    <button
                                        key={t.id}
                                        onClick={() => setSelectedTemplate(t.id)}
                                        className={`flex-shrink-0 w-24 h-32 rounded-xl overflow-hidden relative border-2 transition-colors ${selectedTemplate === t.id ? 'border-violet-500' : 'border-transparent'}`}
                                    >
                                        <img src={t.img} alt={t.category} className="w-full h-full object-cover object-top" />
                                        {selectedTemplate === t.id && <span className="absolute top-1.5 right-1.5 w-5 h-5 bg-violet-600 rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></span>}
                                    </button>
                                ))}
                            </TemplateRow>
                        </div>
                    ...
                    <div className="px-4 pb-5 pt-3 border-t border-gray-100 bg-white">
                        <button
                            ...
                            disabled={generating}
                            className={`w-full py-3.5 rounded-2xl text-sm cursor-pointer font-semibold text-white transition-all flex items-center justify-center gap-2 disabled:opacity-60 ${generating ? 'bg-gray-400' : 'bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-700 hover:to-violet-600'}`}
                        >
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center px-6">
                        <div className="flex items-center gap-3 mb-9">
                            <div className="w-44 h-56 bg-gray-100 rounded-2xl overflow-hidden shadow-lg -rotate-3">
                                <img src={uploadedImage || VID_BEFORE} alt="before" className="w-full h-full object-cover" />
                            </div>
                    ...
                    <DropdownBelow anchorRef={headerRef} width={460}>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-4">
                            {RECENT_TOOL_IDS.map(id => {
                                ...
                                if (!tool) return null;
                                return <ToolCard key={`recent-${tool.id}`} tool={tool} active={tool.id === 'video'} onClick={handleToolClick} />;
                            })}
                        </div>
                        <p className="text-xs font-semibold text-gray-500 px-1 mb-2">All tools</p>
                        <div className="grid grid-cols-2 gap-2">
                            {TOOL_LIST.map(tool => (
                                <ToolCard key={tool.id} tool={tool} active={tool.id === 'video'} onClick={handleToolClick} />
                            ))}
                        </div>
                    </DropdownBelow>
                </>
                        ...
                        <div className="flex-1 min-h-0 px-6 pb-6 pt-1 overflow-y-auto">
                            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                                {filteredTemplates.map(t => (
                                    <button
                                        key={t.id}
                                        onClick={() => { setSelectedTemplate(t.id); setSeeAllOpen(false); }}
                                    ...
                                    >
                                        <img src={t.img} alt={t.category} className="w-full h-full object-cover object-top" />
                                        {selectedTemplate === t.id && <span className="absolute top-2 right-2 w-5 h-5 bg-violet-600 rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></span>}
                                    </button>
                                ))}
                            </div>
                        </div>
