  ...
  return (
    <div className="bg-surface border border-gray-100 rounded-2xl shadow-sm h-full flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col flex-1 px-3 py-3 gap-2">
        {QUICK_ACTIONS.map(
          ({ href, icon: Icon, label, description, iconBg, iconColor }) => (
            <Link
              key={href}
              href={href}
            ...
            >
              ...
              {/* Arrow */}
              <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-gray-500 group-hover:translate-x-0.5 transition-all duration-150 shrink-0" />
            </Link>
          ),
        )}
      </div>
