...
import {
    ...
    Target, TrendingUp, MoreVertical, X, CalendarDays, LayoutGrid, Zap
} from 'lucide-react';
import { useRouter } from 'next/navigation';

/* ── Design tokens (explicit whites — white card on #f7f8fc page) ── */
const C = {
    white: '#ffffff',
    pageBg: '#f7f8fc',
    border: '#e5e7eb',
...
const SocialMediaPlanner = () => {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('selectedsocialForScheduling');
        if (stored) {
            const socialData = JSON.parse(stored);
            if (Date.now() - socialData.timestamp < 5000) {
                setSelectedSocial(socialData);
                setShowScheduleModal(true);
                setScheduleType('social');
                initFormValues(socialData);
            }
    ...
    const initFormValues = (social = null) => {
        const defaults = {};
        Object.keys(PLATFORM_CONFIG).forEach(p => {
            defaults[`${p}_date`] = social?.date || formatDate(new Date());
            defaults[`${p}_time`] = social?.time || '09:00';
        });
    ...
    const goToToday = () => { setCurrentDate(new Date(2025, 8, 23, 7, 21, 0)); setSelectedDay(null); };
    ...
    const handleSchedule = () => {
        ...
        Object.keys(PLATFORM_CONFIG).forEach(platform => {
            ...
            if (dateVal && timeVal) {
                newItems.push({
                    ...selectedSocial,
                    id: `${Date.now()}-${Math.random()}-${platform}`,
    ...
    const handleCreateAd = () => router.push('/creatives/social-creatives/create');
    const handleCreatePost = () => router.push('/create-social-creatives');
                        ...
                        >
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    color: isToday ? 'rgba(255,255,255,0.7)' : C.textTertiary,
                                }}>
                                    {FULL_DAYS[idx].slice(0, 3)}
                                </div>
                                <div style={{
                                    fontSize: 20, fontWeight: isToday ? 700 : 400, lineHeight: 1,
                                    color: isToday ? '#fff' : isSelected ? C.blue : !isCurrentMonth ? C.textTertiary : C.textPrimary,
                                }}>
                                    {date.getDate()}
                                </div>
                            </div>
                            <div style={{ padding: '8px 6px', overflowY: 'auto' }}>
                                ...
                                {isToday && items.length === 0 && (
                                    <div style={{ fontSize: 10, color: C.textTertiary, textAlign: 'center', marginTop: 10 }}>
                                        No posts today
                                    </div>
                                )}
                            </div>
    ...
    return (
        <div style={{ fontFamily: 'inherit', color: C.textPrimary }}>
            ...
            {/* Page header */}
            <div onClick={() => router.push('/studio')}
                className=' hover:scale-105 transition-all duration-200' style={{ padding: '24px 0 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <button style={{ padding: 8, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, cursor: 'pointer', color: C.textSecondary, display: 'flex' }}>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            ...
                            <button onClick={goToToday} style={{ ...navBtn, padding: '5px 10px', fontSize: 12, fontWeight: 500, color: C.textPrimary }}>Today</button>
                        ...
                        {sidebarTab === 'goals' && (
                            <div style={{ padding: '16px 14px' }}>
                                ...
                                <div style={{ background: C.pageBg, borderRadius: 10, padding: 14, border: `1px solid ${C.border}` }}>
                                    ...
                                    <p style={{ fontSize: 12, color: C.textSecondary, margin: '0 0 12px', lineHeight: 1.6 }}>
                                        Consider boosting a recent post so audiences that don't follow you may see it.
                                    </p>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(2px)' }}>
                    <div style={{ background: C.white, borderRadius: 14, width: '90%', maxWidth: 480, border: `1px solid ${C.border}`, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
                        ...
                        <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div>
                                <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: C.textPrimary }}>
                                    Schedule {selectedSocial?.name || 'Ad'}
                                </h2>
                                <p style={{ margin: '4px 0 0', fontSize: 12, color: C.textSecondary }}>
                                    Set the date and time for each platform.
                                </p>
                            </div>
                            <button onClick={() => setShowScheduleModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.textTertiary, padding: 2 }}>
                                <X size={16} />
                            </button>
                        </div>
