...
import {
  Globe, Image, Loader2, X, CheckCircle2, ChevronRight,
  FileUp, Wand2, LayoutTemplate, Target, Images,
} from "lucide-react";
...
const INSPIRE_PROMPTS = [
  "A vibrant digital flyer for a summer music festival with bold typography",
  "A professional print flyer for a business leadership seminar",
  "A colorful community event flyer with a warm, welcoming feel",
  "An elegant wedding announcement flyer with gold accents",
  "A bold promotional flyer for a limited-time product sale",
  "A minimalist startup launch flyer with clean lines and modern fonts",
];

const STEPS = [
  { id: 1, label: "Brand Details",   icon: Image },
  { id: 2, label: "Goals & Layout",  icon: Target },
...
const T = {
  ...
  importBg:     "bg-purple-50/40",
  importBorder: "border-purple-100",
  stepActive:   "border-purple-600 bg-purple-600 text-white",
  stepCurrent:  "border-purple-600 text-purple-600 bg-surface",
  connector:    "bg-purple-600",
  pill:         "border-purple-600 bg-purple-50 text-purple-700",
...
const FlyerForm = ({
  ...
  // Brand details (local + synced to formData)
  ...
  const [brandColor,     setBrandColor]     = useState(activeBrand?.primary_color || "#059669");
  ...
  // Keep formData in sync with local brand fields
  useEffect(() => {
    setFormData((p) => ({
      ...p,
      brandName,
  ...
  }, [brandName, description, brandColor, font, logo]);
  ...
  const handleImportBrand = async () => {
    ...
    try {
      ...
      setBrandColor(d.primary_color || "#059669");
  ...
  const handleContinue = () => {
    ...
    if (step === 1 && !description.trim())  return setError("Please enter a flyer description.");
      ...
      <div className="bg-surface rounded-lg p-2 flex flex-col gap-5">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-5">
            ...
            <Field label="Flyer Description" required>
              <div className="relative">
                <textarea
                  value={description}
                  onChange={(e) => { setDescription(e.target.value); setError(""); }}
                  placeholder="Describe your flyer… event name, key message, tone, and any special details."
                  rows={4}
                  className={`${inputCls} placeholder:text-xs placeholder:text-gray-400 resize-none pb-10`}
                />
                <button
                  onClick={handleInspire}
                  className="absolute bottom-3 left-3 text-xs font-semibold bg-surface border border-gray-200 text-gray-500 hover:border-purple-400 hover:text-purple-600 px-3 py-1 rounded-lg cursor-pointer transition-all flex items-center gap-1.5"
                >
                  ✨ Inspire Me
                </button>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {BRAND_COLORS.map((hex) => (
                      <button
                        ...
                        className={`w-7 h-7 rounded-md cursor-pointer border-2 transition-transform hover:scale-110 ${
                          brandColor === hex ? "border-gray-800 scale-110" : "border-transparent"
                        }`}
                  ...
                  <div className="flex items-center gap-2 flex-none">
                    ...
                    <input
                      ...
                      value={brandColor}
                      onChange={(e) =>
                        /^#[0-9a-fA-F]{0,6}$/.test(e.target.value) && setBrandColor(e.target.value)
                      }
              ...
              <Field label="Logo">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-2.5 border cursor-pointer border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-purple-500 hover:text-purple-600 flex items-center gap-2 transition"
                  >
                  ...
                  {logo && (
                    <div className="relative group w-10 h-10 shrink-0">
                      <img src={logo} alt="Logo" className="w-full h-full object-contain rounded-lg border border-gray-200" />
                      <button
        ...
        {/* ═══ STEP 2 — Goals & Layout (combined old steps 2+3) ════════════ */}
        {step === 2 && (
          <div className="flex flex-col gap-5">
            <SectionTitle>Goals &amp; Layout</SectionTitle>
              ...
              <div className="flex flex-wrap gap-2">
                {CAMPAIGN_GOAL_OPTIONS.map((g) => (
                  ...
                  >
                    <span>{g.emoji}</span>
                    {g.label}
                    {campaignGoal === g.value && <CheckCircle2 className="w-3 h-3 text-purple-600 ml-0.5" />}
                  </button>
                ))}
              </div>
            </Field>
                    ...
                    <div className="flex items-center justify-center h-10">
                      <div
                        ...
                        style={{
                          width:  `${o.w * 0.9}px`,
                          height: `${o.h * 0.9}px`,
                          background: orientation === o.value ? "#ecfdf5" : "#f9fafb",
                        }}
                      />
                    </div>
        ...
        {/* ═══ STEP 3 — Background Image (mirrors ImageAdsForm step 3) ═════ */}
        {step === 3 && (
          <div className="flex flex-col gap-4">
            ...
            {/* ── Upload / picker zone ── */}
            <div
              className={`border-2 border-dashed ${T.importBorder} rounded-2xl p-8 ${T.importBg} flex flex-col items-center gap-3 cursor-pointer hover:border-purple-400 hover:bg-purple-50/60 transition-all`}
              onClick={() => setMediaPickerOpen(true)}
            >
              <div className="w-10 h-10 bg-surface border border-purple-200 rounded-xl flex items-center justify-center shadow-sm">
          ...
          {step < STEPS.length ? (
            ...
            >
              {generating
                ...
                : <><Wand2 className="w-4 h-4" /> Generate Flyers</>
              }
            </button>
          )}
        </div>
...
const inputCls = "w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent";
