...
import {
    GitCompareArrows, Upload, Globe, Loader2, X,
    CheckCircle, XCircle, TrendingUp, TrendingDown,
    AlertCircle, Image as ImageIcon, Plus,
    LayoutGrid,
    Check,
} from "lucide-react";
...
/* ─── constants ───────────────────────────────────────────────── */
const MODES = [
    { id: "creatives", label: "Compare Creatives", icon: Upload, description: "Upload two ad creatives" },
    { id: "websites", label: "Compare Websites", icon: Globe, description: "Enter two competitor URLs" },
];
...
const scoreBg = (s) =>
    s >= 80 ? "bg-emerald-50 text-emerald-700" : s >= 60 ? "bg-blue-50 text-blue-700" : s >= 40 ? "bg-orange-50 text-orange-700" : "bg-red-50 text-red-700";

const marginColor = { Narrow: "text-blue-600 bg-blue-50", Clear: "text-orange-600 bg-orange-50", Decisive: "text-red-600 bg-red-50" };
        ...
        <div className="mb-4">
            <div className="flex justify-between items-center mb-1.5">
                ...
                <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${scoreBg(scoreA)}`}>{scoreA}</span>
                    <span className="text-xs text-gray-300">vs</span>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${scoreBg(scoreB)}`}>{scoreB}</span>
                </div>
            </div>
            ...
            {diff !== 0 && (
                <p className="text-[10px] text-gray-400 mt-0.5 text-right">
                    {diff > 0 ? `${labelA} leads by ${diff.toFixed(1)}` : `${labelB} leads by ${Math.abs(diff).toFixed(1)}`}
                </p>
            )}
        </div>
    );
...
function ProConList({ pros = [], cons = [] }) {
    return (
        <div className="space-y-1.5">
            {pros.map((p, i) => (
                <div key={`pro-${i}`} className="flex gap-2 items-start p-2 rounded-lg bg-emerald-50 border border-emerald-100">
                    <TrendingUp className="w-3 h-3 text-emerald-500 shrink-0 mt-0.5" />
                    <p className="text-xs text-emerald-800 leading-relaxed">{p}</p>
                </div>
            ))}
            {cons.map((c, i) => (
                <div key={`con-${i}`} className="flex gap-2 items-start p-2 rounded-lg bg-red-50 border border-red-100">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            {(item.desktopShare != null) && (
                <div className="col-span-2 bg-gray-50 border border-gray-100 rounded-xl p-2.5">
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide font-medium mb-1.5">Device Split</p>
                    <div className="flex gap-3 items-center">
                        <div className="flex-1">
                            <div className="flex justify-between text-[10px] text-gray-500 mb-1">
                                <span>Desktop</span><span>{item.desktopShare}%</span>
                            </div>
                            <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${item.desktopShare}%` }} />
                            </div>
                        </div>
                        <div className="flex-1">
                            <div className="flex justify-between text-[10px] text-gray-500 mb-1">
                                <span>Mobile</span><span>{item.mobileShare}%</span>
                            </div>
                            <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full bg-violet-500 rounded-full" style={{ width: `${item.mobileShare}%` }} />
                            </div>
                        </div>
                    </div>
                </div>
...
function MiniDesignCanvas({ canvasData, elements, maxW = 120, maxH = 80 }) {
        ...
        (elements || []).forEach((el) => {
            ...
            if (el.type === "shape") {
                ...
                if (el.shape === "circle") {
                    ...
                    if (r) { ctx.beginPath(); ctx.roundRect(el.x, el.y, el.width, el.height, r); ctx.fill(); }
            ...
            if (el.type === "text") {
                ...
                ctx.fillText(text.split(" ").slice(0, 4).join(" "), x, el.y + (el.fontSize || 14));
            }
    ...
    return <canvas ref={canvasRef} style={{ width: canvasData.width * scale, height: canvasData.height * scale, display: "block" }} />;
}
...
function normalizeDesign(raw) {
    ...
    if (parsed && typeof parsed === "object") {
        if (parsed.canvas && Array.isArray(parsed.elements)) { canvasData = parsed.canvas; elements = parsed.elements; }
...
function CreativeUpload({ label, value, onChange }) {
    ...
    const acceptFile = (f) => {
        ...
        reader.onload = (e) => onChange({ ...value, url: e.target.result, file: f });
            ...
            {activeTab === "upload" && (
                !value.url || value.url.startsWith("canvas://") ? (
                    <div
                        ...
                        style={{ borderColor: dragging ? "#6366f1" : "#e5e7eb", background: dragging ? "#eef2ff" : "#f9fafb" }}
                    >
                        <input ref={fileRef} type="file" accept="image/*" onChange={(e) => acceptFile(e.target.files?.[0])} className="hidden" />
                        ...
                        <p className="text-xs text-gray-400 mt-1">PNG, JPG, WEBP · drag & drop</p>
                    </div>
                ) : (
                ...
                <div className="flex-1 flex flex-col min-h-0">
                                    ...
                                    >
                                        ...
                                        <div className="px-2 py-1.5 bg-surface">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                {d.score > 0 && (
                                                    <span className="text-[10px] font-bold text-green-600">★ {d.score}</span>
                                                )}
                                            </div>
                                        </div>
                                    </button>
                    ...
                    {selectedDesignId && (
                        <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-xl">
                            ...
                            <button onClick={clear} className="text-indigo-400 hover:text-indigo-600 cursor-pointer transition">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-indigo-50 via-white to-violet-50 border border-indigo-100 rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {marginOfVictory && (
                            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${marginColor[marginOfVictory] ?? "text-gray-600 bg-gray-100"}`}>
            ...
            <div className="bg-surface border border-gray-200 rounded-2xl p-5 ">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                    {[{ item: itemA, which: "A" }, { item: itemB, which: "B" }].map(({ item, which }) => (
                        <div
                            key={which}
                            className={`rounded-xl p-4 border-2 text-center ${winner === which ? "border-emerald-200 bg-emerald-50" : "border-gray-100 bg-gray-50"}`}
                        >
                            <p className="text-xs text-gray-500 mb-1 truncate font-medium">{item.label}</p>
                            <p className="text-3xl font-black mb-2" style={{ color: scoreColor(item.overallScore) }}>
                                {item.overallScore}
                            </p>
...
export default function Comparison() {
    ...
    // Creatives
    const [creativeA, setCreativeA] = useState({ url: "", file: null, label: "Creative A" });
    const [creativeB, setCreativeB] = useState({ url: "", file: null, label: "Creative B" });

    // Websites
    const [urlA, setUrlA] = useState("");
    const [urlB, setUrlB] = useState("");

    const canCompare = mode === "creatives"
            ...
            <div className="pb-5">
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 bg-gradient-to-br from-indigo-500 to-violet-600 ">
                            <GitCompareArrows className="w-6 h-6 text-white" />
                        </div>
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Creative Comparison</h1>
                            <p className="text-sm text-gray-400">Compare two creatives or competitor websites with AI-powered scoring.</p>
                        </div>
                    </div>
                </div>
                    ...
                    {MODES.map((m) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div>
                                    <p className={`text-sm font-semibold ${active ? "text-indigo-700" : "text-gray-700"}`}>{m.label}</p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_56px_1fr] gap-4 items-stretch mb-6">
                        {mode === "creatives" ? (
                            <>
                                ...
                                <CreativeUpload label="Creative B" value={creativeB} onChange={setCreativeB} />
                            </>
                        ) : (
                            <>
                                ...
                                <WebsiteInput label="Website B" value={urlB} onChange={setUrlB} />
                            </>
                        )}
                    </div>
                )}
                ...
                {!loading && !result && (
                    <div className="flex justify-center mb-10">
                        <button
                            onClick={handleRun}
                            disabled={!canCompare}
                            className="flex items-center gap-2 px-8 py-3.5 rounded-2xl text-sm font-semibold text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all hover:scale-105 active:scale-95 cursor-pointer  bg-gradient-to-r from-indigo-500 to-violet-600"
                        >
                ...
                {loading && (
                    <div className="flex items-center justify-center py-24">
                        <div className="text-center">
                            ...
                            <p className="text-gray-400 text-sm">AI is analyzing and comparing…</p>
                        </div>
                    </div>
                )}
