...
const ThumbnailsCreativeWizard = () => {
  ...
  const [postData, setPostData] = useState({
    ...
    caption: '',
    hashtags: [],
    size: '1280x720', // YouTube thumbnail default
    campaignGoal: '',
    fileFormat: 'PNG',
    primaryColor: '#000000',
  ...
  useEffect(() => {
    if (activeBrand) {
      setPostData(prev => ({
        ...
        caption: prev.caption || `Amazing thumbnail for ${activeBrand.name}!`,
        hashtags: prev.hashtags.length > 0 ? prev.hashtags : ['#YouTube', '#Thumbnail', '#Viral'],
      }));
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/pexels?query=${encodeURIComponent(`${postData.brandName} ${postData.campaignGoal} youtube thumbnail`)}&per_page=20`);
        const data = await res.json();
        setRecommendedImages((data.photos || []).map(p => ({
          id: p.id,
          src: p.src.medium,
          large: p.src.large2x,
          alt: p.alt || 'Recommended thumbnail',
        })));
  ...
  // Reusable handleApplySelected — fixes CORS + works with all sources
    ...
    try {
      const processed = await Promise.all(
        images.map(async (item, i) => {
          ...
          const file = new File([blob], `thumb-${i}.png`, { type: blob.type });
  ...
  return (
    <div className="px-14">
      ...
      <div className="font-medium text-xl">Create YouTube Thumbnails</div>
                ...
                <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">Select or generate a background</p>
                    </div>
                  </div>
                    ...
                    <div className="columns-3 sm:columns-4 md:columns-5 gap-4 space-y-4">
                      {croppedImages.filter(Boolean).map((item, i) => (
                        <div key={i} className="relative group break-inside-avoid mb-4 rounded-lg overflow-hidden border-2 border-blue-700 shadow-md hover:shadow-lg transition-shadow">
                          <img src={item.previewUrl} alt="selected" className="w-full h-auto object-cover rounded-md" loading="lazy" />
                          <button
                            onClick={() => {
                              setCroppedImages(prev => prev.filter((_, idx) => idx !== i));
                              if (i <= currentCropIndex && currentCropIndex > 0) setCurrentCropIndex(prev => prev - 1);
                            }}
                            className="absolute top-2 cursor-pointer right-1 opacity-0 group-hover:opacity-100 transition-all bg-red-600 hover:bg-red-700 text-white rounded-full p-2 shadow-lg z-10"
                          >
      ...
      <SocialIntegrationModal
        ...
        onContinue={() => setIsSocialModalOpen(false)}
