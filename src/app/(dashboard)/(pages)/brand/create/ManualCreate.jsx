...
import { Star, Share2, Recycle, CheckCircle2, Facebook, Instagram, Globe, Loader2, Check } from "lucide-react";
...
export default function ManualCreate({
  ...
  useEffect(() => {
    if (step === 4 && createLandingPage && !landingPageCreated) {
      ...
      showNotification(
        ...
        'Please wait while we build your landing page...',
        'info'
      );
      ...
      const timer = setTimeout(() => {
        ...
        setTimeout(() => {
          showNotification(
            ...
            'Your landing page is ready to preview.',
            'success'
          );
        }, 100);
      }, 2000);
  ...
  const onLogoFileChange = (e) => {
    ...
    // Check size (500 KB = 500 * 1024 bytes)
    if (file.size > 500 * 1024) {
      showNotification(
        'Logo Too Large',
        'Please choose a logo smaller than 500 KB.',
        'error'
      );
      return;
    }
  ...
  const completeBrandCreation = async () => {
    ...
    if (!compiledData.industry) {
      showNotification(
        'Industry Required',
        'Please select an industry for your brand.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      const brandData = await createManualBrand(payload);
      console.log('Brand created:', brandData);

      // Save to local storage and update context
      const savedBrand = {
        id: brandData.id || Date.now().toString(),
        ...payload,
      ...
      // Navigate after success notification
      setTimeout(() => {
      ...
      setTimeout(() => {
        showNotification(
          'Creation Failed',
          err.message || 'Failed to create brand. Please try again.',
          'error'
        );
      }, 100);
    }
  ...
  return (
    <div className="">
                      ...
                      {[
                        ...
                        <div
                          ...
                          onClick={() => {
                            ...
                            setSelectedWebsiteType(option.id);
                          }}
          ...
          {(step < steps.length || (step === 4 && !createLandingPage)) && (
            <div className="flex justify-between items-center">
              {step === 1 && (
                <button
                  onClick={resetForm}
                  className="border border-gray-200 cursor-pointer text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition duration-300 flex items-center gap-2"
                >
                  Cancel
                </button>
              )}
              {step > 1 && (
                <button
                  ...
                  className="border border-gray-200 cursor-pointer text-gray-600 px-6 py-2 rounded-lg hover:bg-gray-100 transition duration-300"
                >
                  Back
                </button>
              )}
              ...
              {step === 4 && createLandingPage && landingPageCreated && (
                <div className="flex gap-4">
                  <button
      ...
      <NotificationModal
        ...
        duration={notification.type === 'info' ? 0 : 3000}
      />
    </div>
  );
}