...
const ImageToVariationsTab = ({
  ...
  const [searchResults, setSearchResults] = useState([
    ...
    },
    {
      ...
      alt: "Brown and white short-coated puppy",
    },
    {
      id: 3,
  ...
  const handleGenerateVariations = async () => {
    if (croppedImages.length === 0)
      return alert("Please crop at least one image first.");
    ...
    const prompt =
      `variation of creative image, different angle, style, lighting, composition, high quality professional ${inputData.style || ""}`.trim();
  ...
  const handleAddToBrand = async (url) => {
    try {
      ...
      if (!res.ok) throw new Error();
      const blob = await res.blob();
      const file = new File([blob], `variation-brand-${Date.now()}.jpg`, {
        type: blob.type,
      });
            ...
            <div className="border border-gray-200 p-3 rounded-lg">
              ...
              <div className="flex mb-10 flex-col space-y-4 border border-gray-200 py-5 bg-gray-50 justify-center items-center">
                ...
                <p className="text-gray-500 text-xs">
                  Choose an image from your brand or your library.
                </p>
              ...
              <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4 space-y-4">
                {outputs.map((variation) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative bg-surface border rounded-lg overflow-hidden transition-all duration-300 ${
                          isSelected
