...
import { Image, MoreVertical, Download, PlusCircle, ArrowLeft, FileSearch, FolderOpen, FileUp } from 'lucide-react';
...
const ImageToVariationsPipelinePage = () => {
  ...
  const { uploadImage, sendUrl, myImages = [], myImagesLoading, fetchMyImages, deleteImage } = useAuth();
  ...
  const [searchResults, setSearchResults] = useState([
    { id: 1, src: "https://images.pexels.com/photos/1108099/pexels-photo-1108099.jpeg", alt: "Puppies" },
    { id: 2, src: "https://images.pexels.com/photos/1805164/pexels-photo-1805164.jpeg", alt: "Puppy" },
    { id: 3, src: "https://images.pexels.com/photos/4588047/pexels-photo-4588047.jpeg", alt: "Party Puppy" },
    { id: 4, src: "https://images.pexels.com/photos/2007/pexels-photo-2007.jpeg", alt: "Cute Dog" },
    { id: 5, src: "https://images.pexels.com/photos/97082/pexels-photo-97082.jpeg", alt: "Snout" },
  ]);
  ...
  const handleFileChange = (e) => {
    ...
    if (!file) return;
    if (!/\.(jpg|jpeg|png|gif|webp)$/i.test(file.name)) return alert('Invalid format');
  ...
  const handleGenerateVariations = async () => {
    ...
    const prompt = `professional creative variation, different angle, lighting, composition, ${inputData.style || 'high quality'}`.trim();
  ...
  const handleAddToBrand = async (url) => {
    const res = await fetch(`/api/proxy-image?url=${encodeURIComponent(url)}`);
    const blob = await res.blob();
    const file = new File([blob], `brand-variation-${Date.now()}.jpg`, { type: blob.type });
  ...
  return (
    <div className="px-14">
        ...
        <div className="overflow-auto space-y-6">
          ...
          {!outputs.length ? (
            <div className="border border-gray-200 p-3 rounded-lg">
              <div className="text-sm flex justify-between border-b p-2 border-b-gray-200 flex-row mb-6">
                <div className="flex justify-center gap-2">
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <p className="text-gray-600 text-xs">Upload an image to generate creative variations.</p>
                  </div>
                </div>
              </div>
                ...
                <div className="columns-2 sm:columns-3 md:columns-4 mb-6">
                  {croppedImages.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={imageSrc[i] || URL.createObjectURL(img)} alt="thumb" className="w-full h-auto object-cover rounded-lg border border-gray-300" />
                      <button onClick={() => {
              ...
              <button onClick={handleGenerateVariations} disabled={loading.generate || croppedImages.length === 0} className="px-3 cursor-pointer mt-6 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-medium disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {loading.generate ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Generate Variations'}
              </button>
            </div>
          ) : (
            ...
            <div className="border border-gray-200 p-3 rounded-lg">
              <div className="sticky top-0 z-50 bg-surface border-b border-gray-200 pb-4 mb-6 flex justify-between items-center">
                ...
                <button onClick={handleBack} className="px-4 py-2 cursor-pointer hover:bg-gray-100 rounded-lg transition">
      ...
      <ImageCropperModal
        ...
        onSkip={() => currentCropIndex < imageSrc.length - 1 ? setCurrentCropIndex(c => c + 1) : setShowCropper(false)}
