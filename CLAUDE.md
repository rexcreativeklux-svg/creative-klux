...
(Last updated 2026-06-18. Keep this short — a running 3–5 item list is fine.)

- **Publish modal rebuilt + made real for canvas designs** — `PublishModal` is now a two-view, category-aware flow (tile picker → edit-in-place platform-native preview) that renders the real creative (image or canvas) and, on publish, snapshots canvas designs to PNG → `uploadImage` → publishes the URL. See the "Publish (My Creations → `PublishModal`)" quirk.
- **Creative category badges fixed** — `saveDesign(brandId, variations, creativeType)` was being called with a hardcoded `"ads"` from **all three** save surfaces (`Adpreview.jsx` `DesignResultPanel`, `ai-chat-page` `PreviewPanel`), so every design saved as Ads. Now the real type is threaded: create-from-url passes `isAds ? 'ads' : 'social'`, `studio/page.jsx` passes `selectedCreative`, ai-chat-page passes the `?creative=` type (`general`→`ads`). `saveDesign` strips the `_creative` suffix. **Existing rows stay mis-tagged** (display reads `raw.type`) — only new saves are correct.
- **Facebook/Meta connect wiring fixed** — `integrations/page.jsx` `resolveIntegrationCredentials` for `facebook`/`instagram`/`meta_ads` returned the raw `{ type:"meta_pages", pages }` object but `handleConnect` read `creds.int_token` (undefined), so it saved an empty integration and never opened the page picker. Now it populates `fbPages` + `pendingFbOauth` and opens `showPageModal` (like google_ads/pinterest_ads), so `handleSelectPlatformPage` saves the chosen Page's token/id. ⚠️ **Connecting still only works for users with a role on the Meta app** — the permissions are on **Standard Access**, not Advanced; non-dev users get "this app isn't available" until App Review grants Advanced Access (+ Business Verification). Needs `FACEBOOK_APP_SECRET` in `.env` for `/api/meta/exchange`.
- **Quota / rate-limit errors surfaced cleanly** — `classifyResult` (`src/utils/errorHelper.js`) now detects 429 / "quota" / "(429)" / "rate limit" (including OpenAI-429-wrapped-in-500) and returns `source:"rate_limit"` with message "You exceeded your current quota, please check your plan and billing details". Custom `Toast` (`src/app/(components)/Toast.jsx`) now **pauses its auto-close timer on hover** (sonner already did).
- **Brand Smart Import double-unwrap fixed** — `brand/import/page.jsx` read `res.data?.data` but `classifyResult` already unwraps once (`data?.data ?? data`), so Import silently no-op'd. Now `res.data?.data || res.data` (the defensive shape every other `sendUrl` caller already uses).
- **Photo Editor rebuilt Photoroom-style** — box-model base image + 8-direction drag-resize, Layers panel, rebuilt Brand Kit / Add text / Templates / Backgrounds / Resize panels. See "Product Photos — Photo Editor". ⚠️ Not yet click-tested or deployed.
