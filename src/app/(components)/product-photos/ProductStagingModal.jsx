...
import { X, Upload, Loader2, MoreHorizontal, Video, ChevronDown, User, Package, Image as ImageIcon, Scissors, Layers, Shirt, Sparkles, LayoutGrid } from 'lucide-react';
...
export default function ProductStagingModal({ onClose, onSwitchTool }) {
    ...
    const handleChangeSomething = () => document.getElementById('product-staging-prompt')?.focus();
    ...
    const handleSaveToGallery = async (url) => {
        ...
        try {
            await saveUrlToGallery(url, uploadImage, { filePrefix: 'product-staged' });
