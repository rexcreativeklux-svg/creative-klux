...
export default function ReuseBrand({ navigateProjectTo, setShowModal }) {
  ...
  const handleCopyUrl = (brand) => {
    ...
    navigator.clipboard
      ...
      .then(() => {
        showNotification(
          "URL Copied!",
          "Brand URL has been copied to clipboard.",
          "success",
          1000
        );
      })
  ...
  const handleDelete = async (brandId) => {
    ...
    showNotification(
      "Deleting Brand",
      "Please wait while we delete your brand...",
      "info",
      0 // Don't auto-close
    );
    ...
    try {
      ...
      if (res) {
        ...
        showNotification(
          "Brand Deleted!",
          "Your brand has been successfully deleted.",
          "success",
          1000
        );
        ...
        showNotification(
          "Deletion Failed",
          "Unable to delete the brand. Please try again.",
          "error",
          2000
        );
      }
  ...
  /* ------------------- Load brands ------------------- */
 useEffect(() => {
  if (token && brands.length === 0 && !brandsLoading) {
    fetchBrands();
  }
}, [token, brands.length]); // ← removed brandsLoading & fetchBrands
              ...
              >
                ...
                <div className="absolute top-2 right-1 z-10">
                  <button
                    ...
                    className="p-1 hover:bg-gray-100 cursor-pointer rounded-full transition"
                  ...
                  {openMenuId === brand.id && (
                    ...
                    >
                      <button
                        className="w-full cursor-pointer text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                        onClick={() => {
                      ...
                      <button
                        ...
                        onClick={() => {
                          try {
                            ...
                            showNotification(
                              "Brand Selected!",
                              `${displayName} is now your active brand.`,
                              "success",
                              2000
                            );
                          } catch (err) {
                            ...
                            showNotification(
                              "Selection Failed",
                              "Failed to select brand. Please try again.",
                              "error",
                              3000
                            );
                          }
                      ...
                      <button
                        className="w-full cursor-pointer text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2 text-red-600"
                        onClick={() => {
