...
import {
    ...
    MoreVertical, MoreHorizontal, List, Grip, ChevronDown,
    Search, ChevronRight, ChevronLeft, Trash2, Edit
} from "lucide-react";
...
export default function TeamsList({ onActiveCountChange, showNotification }) {
    ...
    const getRoleColor = (role) => {
        switch (role?.toLowerCase()) {
            case "owner": return "bg-blue-700 text-white border-blue-200";
    ...
    const handleDeleteTeamClick = async (id, name) => {
        ...
        try {
            ...
            showNotification("Success", result.message || `${name} removed successfully.`, "success", 1000);
            // No need to update local state — context already updated via fetchTeams()
        } catch (err) {
            showNotification("Error", err.message || "Failed to remove member.", "error");
        }
    };
        ...
        <div className=" ">
                ...
                <div className="text-center py-0">
                    <div className="max-w-md mx-auto">
                        ...
                        >
                            ...
                            {/* Background */}
                            <circle cx="150" cy="150" r="140" fill="url(#bgGradient)" opacity="0.5" />
                            ...
                            <g opacity="0.2" stroke="#155dfc" strokeWidth="2">
                                ...
                                <line x1="210" y1="150" x2="150" y2="220">
                                    <animate attributeName="stroke-opacity" values="0.2;0.6;0.2" dur="3s" begin="2.5s" repeatCount="indefinite" />
                                </line>
                            </g>
                            ...
                            <g filter="url(#glow)">
                                ...
                                <path d="M143 85 Q150 90 157 85" stroke="#155dfc" strokeWidth="3" fill="none" strokeLinecap="round" />
                                <circle cx="150" cy="80" r="32" fill="none" stroke="#155dfc" strokeWidth="2" opacity="0.3">
                            ...
                            <g>
                                {/* Left Node */}
                                <circle cx="90" cy="150" r="24" fill="url(#nodeGradient)" opacity="0.9" />
                                <circle cx="90" cy="150" r="21" fill="white" />
                                <circle cx="85" cy="147" r="4" fill="#155dfc" />
                                <circle cx="95" cy="147" r="4" fill="#155dfc" />
                                <path d="M85 154 Q90 157 95 154" stroke="#155dfc" strokeWidth="2" fill="none" />

                                {/* Right Node */}
                                <circle cx="210" cy="150" r="24" fill="url(#nodeGradient)" opacity="0.9" />
                                <circle cx="210" cy="150" r="21" fill="white" />
                                <circle cx="205" cy="147" r="4" fill="#155dfc" />
                                <circle cx="215" cy="147" r="4" fill="#155dfc" />
                                <path d="M205 154 Q210 157 215 154" stroke="#155dfc" strokeWidth="2" fill="none" />

                                {/* Bottom Node */}
                                <circle cx="150" cy="220" r="24" fill="url(#nodeGradient)" opacity="0.9" />
                                <circle cx="150" cy="220" r="21" fill="white" />
                                <circle cx="145" cy="217" r="4" fill="#155dfc" />
                                <circle cx="155" cy="217" r="4" fill="#155dfc" />
                                <path d="M145 224 Q150 227 155 224" stroke="#155dfc" strokeWidth="2" fill="none" />
                            </g>
                        ...
                        <p className="text-gray-600 mb-6 text-sm leading-relaxed max-w-sm mx-auto">
                            Build your dream team! Invite your first member using the form above to start collaborating.
                        </p>
                                    ...
                                    <div className="flex justify-between items-center pb-2 gap-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="gap-2">
                                                ...
                                                <span className={`inline-flex px-2 rounded-lg text-xs font-medium border ${getRoleColor(team.role)}`}>
                                                    {team.role || "No Role"}
                                                </span>
                                    ...
                                    <div className="mt-4 flex flex-col gap-3">
                                        ...
                                        <div className="flex justify-between">
                                            <div className="flex gap-2 border-t border-t-gray-200 pt-2">
                                                <a href={`mailto:${team.email}`} className="text-gray-400 hover:text-blue-500 cursor-pointer"><Mail size={16} /></a>
                                                <a href={`tel:${team.phone || ""}`} className="text-gray-400 hover:text-blue-500 cursor-pointer"><Phone size={16} /></a>
                                                <a href={`https://wa.me/${team.phone || ""}`} className="text-gray-400 hover:text-blue-500 cursor-pointer"><MessageCircle size={16} /></a>
                                                <a href="https://facebook.com/default" className="text-gray-400 hover:text-blue-500 cursor-pointer"><FaFacebook size={16} /></a>
                                            </div>
                                    ...
                                    {paginatedTeams.map((team) => (
                                        <tr key={team.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full border ${getRoleColor(team.role)}`}>
                                                    {team.role || "No Role"}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full border ${getStatusColor(team.status)}`}>
                                                    {getStatusLabel(team.status)}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4">
                                                <div className="flex items-center gap-2">
                                                    <button title="edit" className="p-1 hover:bg-gray-100 rounded cursor-pointer">
            ...
            <div className="flex justify-between items-center mt-13">
                <span className="text-sm text-gray-600">
                    Showing {(currentPage - 1) * entriesPerPage + 1}–{Math.min(currentPage * entriesPerPage, filteredTeams.length)} of {filteredTeams.length}
                </span>
                ...
                <div className="flex gap-2">
                    ...
                    {Array.from({ length: Math.ceil(filteredTeams.length / entriesPerPage) }, (_, i) => (
                        <button
                            key={i}
