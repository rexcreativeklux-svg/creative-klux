...
export default function Sessions() {
    ...
    const fetchSessions = async () => {
      try {
        const res = await fetch(
          "https://api.creativeklux.com/creativeklux-frontend/user/sessions",
          {
            headers: {
  ...
  const handlePasswordUpdate = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.creativeklux.com/creativeklux-frontend/user/${user?.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.creativeklux.com/creativeklux-frontend/user/delete/${user.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleTerminate = async (sessionId) => {
    try {
      const res = await fetch(
        `https://api.creativeklux.com/creativeklux-frontend/user/sessions/${sessionId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      <div className="border-b pb-3 border-b-gray-200">
        <div className="space-y-2">
          <h1 className="font-semibold text-2xl">
            Active sessions & passwords
          </h1>
          <p className="text-gray-600">
            Manage your active sessions, update password, or delete your
            account.
          </p>
        </div>
      </div>
