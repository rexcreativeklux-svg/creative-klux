...
import {
  ChevronLeft, ChevronRight, CalendarDays, Plus, Trash2, Filter, Search, X
} from 'lucide-react';
import {
        ...
        <div className="flex items-center border-b border-gray-200 bg-surface rounded-t-xl px-2 shadow-sm">
          {STATUS_TABS.map(tab => (
            <button
              ...
              className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors whitespace-nowrap cursor-pointer ${statusFilter === tab.key
            ...
            >
              ...
              <span
                ...
                style={statusFilter === tab.key ? { backgroundColor: '#003dda', color: '#fff' } : { backgroundColor: '#f3f4f6', color: '#6b7280' }}
              >
      ...
      {posts.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-surface border-x border-b border-gray-200 rounded-b-xl mb-6 shadow-sm">
            ...
            >
              ...
              <optgroup label="── Social Media">
                {platformOptions.map(p => {
                  ...
                  const name = p.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                  return <option key={p} value={p}>{m.emoji} {name}</option>;
                })}
              </optgroup>
            </select>
          </div>
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 cursor-pointer">
              ...
              {days.map((day, idx) => {
                ...
                return (
                  <div
                    ...
                    className={
                      'min-h-[90px] p-1.5 border-b cursor-pointer border-r border-gray-100 cursor-pointer transition-colors ' +
                      (!inMonth    ? 'opacity-30 '      : '') +
          ...
          <div className="rounded-xl border border-gray-200 bg-surface p-4 sticky top-6">
            {selectedDay ? (
              <>
                ...
                {selectedDayPosts.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-xs text-gray-400">No posts on this day</p>
                    <Link href="/creatives">
              ...
              <div className="text-center py-10">
                ...
                <p className="text-xs text-gray-400">Click a day to see its posts</p>
              </div>
            )}
          </div>
        </div>
