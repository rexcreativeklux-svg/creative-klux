...
const QUALITY_TIERS = [
    {
        ...
        img: px(6780091),
        features: ['4k+ resolution', 'Best product accuracy', 'Most realistic results', 'Highest quality', 'Consumes most credits'],
    },
    {
        id: 'High', name: 'Advanced', tag: 'Max', tagColor: 'bg-indigo-100 text-indigo-700',
        img: px(6780038),
        features: ['2k resolution', 'Better product accuracy', 'Realistic results', 'High quality', 'Consumes more credits'],
    },
    {
...
const TOOL_CONFIGS = {
    staging: {
        ...
        prompt: 'Professional product staging photography. Place the product from the reference image into a realistic lifestyle scene that tells a story and shows it in use. Keep the product identical — same shape, color and label. Photorealistic, commercial quality, natural lighting.',
    },
    mannequin: {
        ...
        prompt: 'Ghost mannequin product photography. Display the garment from the reference image on an invisible 3D mannequin form so it shows its natural worn shape. Keep the garment identical — same design, color and pattern. Clean white background, professional e-commerce styling.',
    },
    beautifier: {
        ...
        prompt: 'Polished professional product photography. Enhance the product from the reference image with clean studio lighting, sharp focus and a refined, uncluttered background. Keep the product identical. High-end commercial quality.',
    },
    flatlay: {
        title: 'Flat Lay',
        ...
        prompt: 'Flat lay product photography. Lay the product from the reference image flat on a clean, neutral surface, neatly arranged and evenly lit, top-down view. Keep the product identical. Minimal, professional styling.',
    },
};
...
export default function ProductToolModal({ toolId, onClose, onSwitchTool }) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const result = await generateImage({ prompt: generationPrompt, existing_image_urls: [fileUrl] });
    ...
    const handleDownload = (url) => {
        ...
        a.href = url; a.download = `${toolId}.png`; a.target = '_blank'; a.click();
    };
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center px-6">
                            <div className="flex items-center gap-3 mb-9">
                                <div className="w-44 h-56 bg-gray-100 rounded-2xl overflow-hidden shadow-lg -rotate-3">
                                    <img src={uploadedImage || cfg.beforeImg} alt="before" className="w-full h-full object-cover" />
                                </div>
                    ...
                    <DropdownBelow anchorRef={headerRef} width={460}>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-4">
                            {RECENT_TOOL_IDS.map(id => {
                                ...
                                if (!tool) return null;
                                return <ToolCard key={`recent-${tool.id}`} tool={tool} active={tool.id === toolId} onClick={handleToolClick} />;
                            })}
                        </div>
                        <p className="text-xs font-semibold text-gray-500 px-1 mb-2">All tools</p>
                        <div className="grid grid-cols-2 gap-2">
                            {TOOL_LIST.map(tool => (
                                <ToolCard key={tool.id} tool={tool} active={tool.id === toolId} onClick={handleToolClick} />
                            ))}
                        </div>
                    </DropdownBelow>
                </>
