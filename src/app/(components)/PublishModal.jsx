...
import {
  ...
  publishToGoogleAds,
  publishToTikTokAds,
  publishToPinterestAds,
  publishToLinkedInAds,
  publishToSnapchatAds,
  publishToYouTube,
  publishToTwitter,
  publishToLinkedIn,
  publishToPinterest,
...
const ADS_ORDER = [
  ...
  "snapchat_ads",
  "pinterest_ads",
]; // "linkedin_ads" disabled — re-add here to restore
...
const DesignVisual = ({ image, canvas, elements, className = "" }) => {
  if (image)
    return (
      <img src={image} alt="" className={`w-full object-cover ${className}`} />
    );
  if (canvas)
    return (
      <div className="w-full bg-gray-50">
        <DesignCanvas canvas={canvas} elements={elements} />
      </div>
    );
  return null;
};
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <p
          className={`text-sm font-semibold truncate ${connected ? "text-gray-900" : "text-gray-500"}`}
        >
          {label}
        </p>
        <p
          className={`text-[11px] truncate ${connected ? "text-green-600" : "text-gray-400"}`}
        >
...
const InstagramPreview = ({
  ...
  <div className="rounded-xl border border-gray-200 bg-surface overflow-hidden shadow-sm">
    ...
    <EditableCaption
      ...
      className="py-2"
      prefix={
        <span className="font-semibold mr-1 align-top leading-snug">
          {name}
        </span>
      }
    />
...
export default function PublishModal({
  ...
  const order = useMemo(
    () => platformsForCategory(creative?.category),
    [creative],
  );
    ...
    async (scheduledUnix = null) => {
      ...
      try {
        ...
        if (!imageUrl && creative.canvas) {
          try {
            ...
            if (file) {
              ...
              const d = up?.data ?? up;
              const pick = (o) =>
                o?.image || o?.image_url || o?.url || o?.path || o?.src || null;
              imageUrl =
                pick(up) || pick(d) || (Array.isArray(d) ? pick(d[0]) : null);
            }
          } catch (err) {
            console.warn("renderDesignToFile/upload failed:", err);
            throw new Error(
              "Couldn't prepare the design image for publishing. Try downloading it first.",
            );
          }
          if (!imageUrl)
            throw new Error(
              "Couldn't prepare the design image for publishing.",
            );
        }
        ...
        if (selected === "facebook") {
          ...
          if (!fb)
            throw new Error(
              "Connect a Facebook Page first — Meta ads run as a Page.",
            );
          if (!adBudget || Number(adBudget) < MIN_AD_BUDGET)
            throw new Error(
              `Daily budget must be at least ${MIN_AD_BUDGET.toLocaleString()}.`,
            );
          ...
          if (!integration.int_refresh_token)
            throw new Error(
              "Reconnect Google Ads — no refresh token stored for this account.",
            );
          if (!adBudget || Number(adBudget) < MIN_AD_BUDGET)
            throw new Error(
              `Daily budget must be at least ${MIN_AD_BUDGET.toLocaleString()}.`,
          ...
          if (!locId)
            throw new Error(
              "Pick a target country that's mapped to a TikTok region.",
            );
  ...
  const scheduleMax = toLocalInput(
  ...
  const toggleSchedule = () => {
    if (!SCHEDULABLE.includes(selected)) {
      showToast(
        "Scheduling is only available for Facebook and YouTube right now",
        "error",
      );
      return;
    }
  ...
  const confirmSchedule = () => {
    ...
    if (ts < now + 10 * 60 * 1000) {
      showToast("Schedule at least 10 minutes from now", "error");
      return;
    }
    if (ts > now + 180 * 24 * 60 * 60 * 1000) {
      showToast("Schedule within 6 months", "error");
      return;
    }
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-gray-600">
                  {PLATFORMS[selected]?.label} preview
                </span>
              ...
              {isAdForm && (
                <div className="rounded-xl border border-gray-200 p-3 flex flex-col gap-3">
                  ...
                  <div
                    className={
                      selected === "meta_ads" ? "grid grid-cols-2 gap-2" : ""
                    }
                  >
                    <label className="flex flex-col gap-1">
                      <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">
                      ...
                      {adBudget !== "" && Number(adBudget) < MIN_AD_BUDGET && (
                        <span className="text-[10px] text-red-500">
                          Minimum daily budget is{" "}
                          {MIN_AD_BUDGET.toLocaleString()}
                        </span>
                      )}
                    </label>
                    {selected === "meta_ads" && (
                  ...
                  {(selected === "meta_ads" ||
                    ...
                    <label className="flex flex-col gap-1">
                      ...
                      {selected === "tiktok_ads" &&
                        !TIKTOK_LOCATION_IDS[adCountry] && (
                          <span className="text-[10px] text-red-500">
                            This country isn’t mapped to a TikTok region yet —
                            pick another.
                          </span>
                        )}
                    </label>
                  ...
                  <p className="text-[10px] text-gray-400">
                    {selected === "meta_ads" ? (
                      <>
                        Ad goes <b>live immediately</b> and spends up to your
                        daily budget. Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}. Other
                        settings (age 18-65, all genders, auto-bid) use smart
                        defaults.
                      </>
                    ) : selected === "google_ads" ? (
                      <>
                        Creates a <b>paused</b> Display campaign + Responsive
                        Display Ad in Google Ads — review &amp; enable it in Ads
                        Manager to start spending. Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}.
                      </>
                    ) : selected === "tiktok_ads" ? (
                      <>
                        Creates a <b>paused</b> TikTok campaign + video ad (your
                        image becomes a short clip) — review &amp; enable it in
                        TikTok Ads Manager to start spending. Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}.
                      </>
                    ) : selected === "linkedin_ads" ? (
                      <>
                        Creates a <b>draft/paused</b> LinkedIn Sponsored Content
                        ad (runs as the Company Page that owns the ad account) —
                        review &amp; activate it in LinkedIn Campaign Manager to
                        start spending. Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}.
                      </>
                    ) : selected === "snapchat_ads" ? (
                      <>
                        Creates a <b>paused</b> Snapchat campaign + single-image
                        Snap ad — review &amp; enable it in Snapchat Ads Manager
                        to start spending. Snap ads are full-screen vertical
                        (9:16). Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}.
                      </>
                    ) : (
                      <>
                        Creates a <b>paused</b> Pinterest campaign + promoted
                        pin — review &amp; enable it in Pinterest Ads Manager to
                        start spending. Sends clicks to{" "}
                        {activeBrand?.url || "your brand website"}.
                      </>
                    )}
              ...
              {(selected === "pinterest" || selected === "pinterest_ads") && (
                <label className="rounded-xl border border-gray-200 p-3 flex flex-col gap-1">
                  ...
                  {pinBoardsLoading ? (
                    <span className="text-sm text-gray-400 flex items-center gap-2 py-1">
              ...
              <div
                className={
                  isAdForm ? "-mx-1 px-1" : "overflow-y-auto -mx-1 px-1"
                }
              ...
              {showSchedule && (
                <div className="rounded-xl border border-gray-200 p-3 flex flex-col gap-2">
                  ...
                  <p className="text-[10px] text-gray-400">
                    {selected === "youtube" ? "YouTube" : "Facebook"} publishes
                    it automatically · 10 min–6 months ahead.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="shrink-0 px-5 py-4 border-t border-gray-100 flex gap-3 justify-end">
              ...
              <button
                ...
                disabled={
                  ...
                  googleAdsIncomplete ||
                  tiktokAdsIncomplete ||
                  pinterestAdsIncomplete ||
                  linkedinAdsIncomplete ||
                  snapchatAdsIncomplete ||
                  pinterestIncomplete
                }
                title={
                  metaAdIncomplete ||
                  ...
                  linkedinAdsIncomplete ||
                  snapchatAdsIncomplete
                    ? "Fill in all ad settings first"
                    : pinterestAdsIncomplete
                      ? "Set a budget and pick a board"
                      : pinterestIncomplete
                        ? "Pick a board first"
                        : undefined
                }
              ...
              >
                {busyAction === "publish" ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />{" "}
                    {isAdForm ? "Creating ad…" : "Publishing…"}
                  </>
                ) : published ? (
                  <>
                    <Check className="w-3.5 h-3.5" />{" "}
                    {isAdForm ? "Ad created!" : "Published!"}
                  </>
                ) : (
                  <>
                    <Send className="w-3.5 h-3.5" />{" "}
                    {isAdForm ? "Create Ad" : "Publish Now"}
                  </>
                )}
              </button>
            </div>
