...
import { Image, Download, CheckCircle2, FileSearch, FolderOpen, FileUp, MoreVertical, ImagePlus, Send, Calendar } from 'lucide-react';
...
const PostsCreativeWizard = () => {
  ...
  const inspirePrompts = [
    'A vibrant Instagram post for a new product launch',
    'A minimalist social media post for a tech startup',
    'A bold promotional post for a fitness campaign',
    'A trendy post for a fashion brand event',
    'A professional post for a corporate announcement',
  ];
  ...
  useEffect(() => {
    if (importedBrand) {
      ...
      console.log('Prefilled post details from URL:', brandUrl);
    }
  }, [importedBrand]);
  ...
  const handleFieldChange = (field, value) => {
    if (field === 'primaryColor' || field === 'secondaryColor') {
      ...
      console.log(`Updated ${field}:`, sanitizedValue);
    } else {
  ...
  const handleInspire = () => {
    ...
    setTimeout(() => {
      ...
      const mockSuggestions = {
        caption: `Exciting post for ${postData.brandName}! ${postData.description.slice(0, 50)}... #${keywords[0] || 'Brand'}`,
        hashtags: [
      ...
      setLoading({ ...loading, 1: false });
      console.log('Inspire suggestions:', mockSuggestions);
    }, 1000);
  };
  ...
  const handleCampaignGoalSelect = (value) => {
    ...
    console.log('Selected campaign goal:', value);
  };
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        const newAsset = { id: Date.now(), src: reader.result, alt: file.name, type: 'image' };
        setCurrentCropAsset(newAsset);
        setShowCropper(true);
        setLoading({ ...loading, 3: false });
        console.log('Triggered crop for uploaded image:', file.name);
      };
      img.onerror = () => {
  ...
  const handleSkipCrop = async () => {
    ...
    setLoading({ ...loading, 3: true });
    const asset = { ...currentCropAsset, id: Date.now() };
    setSearchResults(prev => [asset, ...prev]);
    setShowCropper(false);
    setCurrentCropAsset(null);
    setCrop({ unit: '%', width: 50, x: 25, y: 25, height: 50 });
    setCompletedCrop(null);
    setLoading({ ...loading, 3: false });
    console.log('Skipped crop, added original to library:', asset.src);
  };
  ...
  const handleContinue = useCallback(() => {
    if (step < steps.length) {
      ...
      try {
        ...
        const generatedPosts = generatePostVariations();
  ...
  const handleBack = () => {
    if (step > 1) {
      ...
      console.log('Back button clicked, setting loading true for step:', step);
      setTimeout(() => {
        setStep(step - 1);
        setResult(null);
        setLoading({ ...loading, [step]: false });
        console.log('Wizard step reverted to:', step - 1);
      }, 1000);
    } else {
      router.push('/creatives/designer-creatives');
      console.log('Navigated back to /creatives/designer-creatives');
    }
  };
    ...
    <div className="px-14">
      ...
      {result ? (
        <div className="flex flex-col overflow-hidden w-full mt-3 justify-between gap-6 bg-white rounded-lg py-4 ">
          <div className="font-medium pb-4 ">Generated Posts</div>
        ...
        <div className="flex flex-row gap-10 w-full">
          ...
          <div className="flex flex-col overflow-hidden w-full mt-3 justify-between gap-6 bg-white rounded-2xl p-4">
                  ...
                  <div className="text-sm flex justify-between border-b p-2 border-b-gray-200 flex-row mb-6">
                    <div className="flex justify-center gap-2">
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        <p className="text-gray-600 text-xs">Select up to 5 images for your social media post.</p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex justify-between p-3 rounded-lg">
              ...
              <button
                onClick={() => {
                  console.log('Finish/Continue button clicked, step:', step);
                  handleContinue();
                }}
