...
const px = (id, w = 400) => `https://images.pexels.com/photos/${id}/pexels-photo-${id}.jpeg?auto=compress&cs=tinysrgb&w=${w}&h=${w}&fit=crop`;
...
const STUDIO = ['#f3efe6', '#ece8de', '#f2eee6', '#f6f0df', '#f0d8c4', '#d9e4d2', '#d7e6f0', '#f3dcd6'];
...
export default function BatchPage() {
    ...
    return (
        <div className="pb-10">
                    ...
                    <div className="flex items-end gap-3 mb-7">
                        {HERO_CAPS.map((src, i) => (
                            ...
                            >
                                ...
                                {i === 2 && <div className="absolute inset-y-0 right-0 w-1.5 bg-gradient-to-l from-violet-400/70 to-transparent" />}
                            </div>
                        ))}
                    </div>

                    <h2 className="text-2xl font-bold text-gray-900 mb-1.5">Edit up to 250 images at once</h2>
                    <p className="text-gray-500 mb-7">Drag and drop images or a folder on this page.</p>

                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => imagesInputRef.current?.click()}
                            className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-700 hover:to-violet-600 text-white text-sm font-semibold transition-all cursor-pointer"
                        >
            ...
            {/* Essentials */}
            <h2 className="text-base font-semibold text-gray-900 mt-10 mb-4">Essentials</h2>
            <ScrollRow>
                {ESSENTIALS.map((e) => (
                    <button
                        key={e.id}
                        onClick={() => toast('Template editing is coming soon.')}
                        className="group/tile flex-shrink-0 w-40 h-40 rounded-2xl overflow-hidden relative cursor-pointer"
                    >
                        <img src={TSHIRT} alt="t-shirt" className="w-full h-full object-cover" />
            ...
            {/* Studio */}
            <h2 className="text-base font-semibold text-gray-900 mt-8 mb-4">Studio</h2>
            <ScrollRow>
                {STUDIO.map((_, i) => (
                    <button
                        key={i}
