...
(Added 2026-06-23. **DISABLED in the UI 2026-06-25 — code retained.**) **⚠️ Currently turned OFF.** Connect failed in testing with LinkedIn's generic **"Bummer, something went wrong"** — the app requests the `r_ads/r_ads_reporting/rw_ads` scopes but doesn't have the **Advertising API (Marketing Developer Platform)** product approved, so LinkedIn refuses the authorization. Since that's a LinkedIn review gate (not a code fix), LinkedIn Ads was **commented out of the two user-facing surfaces**: the integrations **card** (+ `LinkedInAdsIcon`) in `integrations/page.jsx` and the **`linkedin_ads` entry in PublishModal `PLATFORMS`** (+ removed from `ADS_ORDER`). **Everything else is left intact and dormant** (dispatcher branch, `resolveLinkedInAdsIntegration`, `publishToLinkedInAds`, the `/api/linkedin-ads/*` routes, label maps, live-fetch branch) — unreachable while the card/modal entry are commented. **To re-add:** uncomment those two spots (after LinkedIn approves the Advertising API). The rest of this section describes the (still-present) implementation.

Separate from organic LinkedIn (which is dormant/person-feed posting). LinkedIn Ads is **Sponsored Content that runs as a Company Page (organization)**, not a person.
...
- **✅ Cross-device rotation write-back — CLOSED (2026-06-23).** The backend now exposes a **PUT `/integrations/{id}`**; `updateIntegration(id, { access_token, refresh_token })` in `AuthContext` calls it. The rotated token is now written back to the backend, not just localStorage: `publishToTwitter`/`publishToTikTok` **return** the rotated `refresh_token` and `PublishModal` persists it via `updateIntegration`; `fetchLivePostsFromConnectedAccounts(integrations, { onTokenRotated })` fires the callback on each X/TikTok rotation, and all six live-fetch pages (social/ads × calendar/publishing/analytics) pass `onTokenRotated: (id, rt) => updateIntegration(id, { refresh_token: rt })`. So a second device's `fetchIntegrations()` now gets the current token. Write-back is **best-effort** (a failed PUT never breaks the publish/fetch; localStorage stays the device-local copy). **⚠️ Assumes a REST PUT accepting `int_token`/`int_refresh_token`** — if the backend uses the Laravel `_method=PUT`-via-POST quirk or different field names, adjust `updateIntegration` only. **Meta/IG/LinkedIn are NOT affected** — their tokens are long-lived and non-rotating.
...
- **Refresh-token model (like X).** TikTok access tokens last **~24h**; the refresh token (~365d) can **rotate**. So the post/posts routes **always refresh first** and **return the rotated refresh token**, and the caller persists it. Stored in **localStorage** (`setStoredTikTokRefresh`/`getStoredTikTokRefresh`, key `ck_tiktok_refresh_<id>`) + sent to the backend as **`int_refresh_token`** in `saveIntegration`. **Cross-device write-back CLOSED 2026-06-23** (see X section): `publishToTikTok` returns the rotated token → `PublishModal` calls `updateIntegration`; the live-fetch branch fires `onTokenRotated` → pages call `updateIntegration` (PUT `/integrations/{id}`). **Token precedence fixed 2026-06-23:** `publishToTikTok` + the live-fetch branch now read **`getStoredTikTokRefresh(id)` FIRST**, backend `int_refresh_token` as fallback (same flip as X — preferring the stale backend copy caused invalid-token failures after a live-fetch rotated it).
...
(Last updated 2026-06-25. Keep this short — a running 3–5 item list is fine.)

- **X/TikTok publish routes: return the rotated token on failure (2026-06-25).** Bug: the publish route refreshes first (which **rotates** the X/TikTok refresh token on the platform side — old one dies), then if a LATER step throws (e.g. X media upload failing with "…does not have any credits"), the route's `catch` **discarded the freshly-rotated token**. So the next attempt reused the now-dead old token → **"Value passed for the token was invalid"** — producing the alternating credits→token-invalid→credits pattern. Fix: hoisted `rotatedRefresh` to function scope in `/api/twitter/post` + `/api/tiktok/post` so the `catch` returns it; `publishToTwitter`/`publishToTikTok` already persist `data.refresh_token` (localStorage) before throwing, so retries now consistently surface the *real* error (X's "no credits" tier wall) instead of a spurious token error. **X posting itself is still blocked by the X API plan** (write credits / Basic+ for media) — that's X-side, not code.

- **X/TikTok cross-device token write-back CLOSED (2026-06-23)** — backend added **PUT `/integrations/{id}`**; new `updateIntegration(id, {access_token, refresh_token})` in `AuthContext`. `publishToTwitter`/`publishToTikTok` now **return** the rotated refresh token (PublishModal persists it), and `fetchLivePostsFromConnectedAccounts(integrations, { onTokenRotated })` fires a callback on rotation that all six live-fetch pages wire to `updateIntegration`. So a rotated token is written back to the backend (not just localStorage) → a second device's `fetchIntegrations()` gets the current token. Best-effort (a failed PUT never breaks publish/fetch). Assumes a REST PUT with `int_token`/`int_refresh_token` fields. See "X (Twitter) integration".
- **LinkedIn Ads disabled in the UI (2026-06-25)** — connect hit LinkedIn's "Bummer, something went wrong" because the app lacks **Advertising API (Marketing Developer Platform)** approval for the `r_ads/rw_ads` scopes (a LinkedIn review gate, not a code bug). Commented out the integrations **card** + the PublishModal **`linkedin_ads`** entry (removed from `ADS_ORDER`); all backing code/routes retained and dormant. Re-add by uncommenting those two spots after approval. See "LinkedIn Ads integration".
- **Pinterest connect fixed for real (2026-06-25)** — root cause of "Authentication failed" was the exchange using the `pina_…` **access token** as the OAuth Basic-auth secret instead of the app **client secret**. Now uses **`PINTEREST_CLIENT_SECRET`** (added to env/Vercel). Social Pinterest connect confirmed working. Same route powers Pinterest Ads.
- **Cross-device token model — X/TikTok refresh token now lives in `int_token` (2026-06-25).** The backend persists/returns **one** token field, `int_token` (GET `/integrations` returns it; it does NOT return `int_refresh_token` — verified: the old value decoded to the X *access* token `…:at:1`). So per backend guidance, for **X/TikTok** we store the **refresh token in `int_token`** (the access token is ephemeral / re-derived every publish): `saveIntegration` writes `int_token = refresh_token` for `twitter`/`tiktok`; `updateIntegration(id,{refresh_token})` (PUT `/integrations/{id}`) writes the rotated token back to `int_token`; reads use **`getStored…Refresh(id)` → `int_token` → `int_refresh_token`** (localStorage-first for same-session, backend `int_token` as the cross-device source). `publishToTwitter`/`publishToTikTok` **return** the rotated token (PublishModal calls `updateIntegration`); `fetchLivePostsFromConnectedAccounts(integrations,{onTokenRotated})` fires a callback for the same. Because `int_token` IS returned on GET, **cross-device now works after a reconnect** (sequential use; concurrent multi-device is inherently racy with a rotating token). **⚠️ Users must RECONNECT X/TikTok once** so `int_token` gets the refresh token (existing records hold the stale access token). **All six live-fetch pages** (social/ads × calendar/publishing/analytics) pass `onTokenRotated: (id, rt) => updateIntegration(id, { refresh_token: rt })`, so rotations from *both* the publish path and the read/live-fetch path are written back to the backend.
- **X (Twitter) publishing blocked by X API tier, not code (2026-06-25).** Publish returns **"…does not have any credits to fulfill this request"** — confirmed via a text-only diagnostic (now reverted) that even a plain tweet fails, so it's not just the media step. The X app has **no write quota**: it needs to be in a Project with an active access tier. **X is dev-billed** (you pay X's API tier; quota is shared across all users) — Free is ~500 posts/mo and **excludes media upload**; image tweets need **Basic ($100/mo)+**. This is unique to X among the socials (others' posting APIs are free; ads put cost on the user).
