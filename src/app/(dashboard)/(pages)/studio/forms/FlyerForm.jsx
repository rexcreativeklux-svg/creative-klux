...
import {
  Image, Loader2, X, ChevronRight, FileUp, Wand2, LayoutTemplate, Target,
} from "lucide-react";
...
const STEPS = [
  ...
  { id: 3, label: "Layout & Output",    icon: LayoutTemplate },
];
...
const T = {
  ...
  importBg:     "bg-emerald-50/40",
  importBorder: "border-emerald-100",
  stepActive:   "border-emerald-600 bg-emerald-600 text-white",
  stepCurrent:  "border-emerald-600 text-emerald-600 bg-white",
  connector:    "bg-emerald-600",
  pill:         "border-emerald-600 bg-emerald-50 text-emerald-700",
...
const FlyerForm = ({ formData, setFormData, activeBrand, showToast, onResult }) => {
  ...
  const [secondaryColor,  setSecondaryColor]  = useState(activeBrand?.secondary_color || "#059669");
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/pexels?query=${encodeURIComponent(`${brandName} ${campaignGoal} flyer`)}&per_page=${count * 2}`);
      ...
      const generated = (data.photos || []).slice(0, count).map((photo, i) => ({
        ...
        alt:     photo.alt || `Flyer variation ${i + 1}`,
        type:    "image",
      }));
      ...
      setError("Flyer generation failed. Please try again.");
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-lg p-2 flex flex-col gap-5">
            ...
            <Field label="Flyer Description" required>
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute bottom-3 left-3 text-xs font-semibold bg-white border border-gray-200 text-gray-500 hover:border-emerald-400 hover:text-emerald-600 px-3 py-1 rounded-lg cursor-pointer transition-all"
                >
                  ✨ Inspire Me
                </button>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 border-2 border-dashed rounded-xl cursor-pointer text-xs font-semibold transition-all ${
                    logo ? `${T.border} ${T.bgLight} ${T.textDark}` : "border-gray-300 text-gray-500 hover:border-emerald-400 hover:text-emerald-600 hover:bg-emerald-50/40"
                  }`}
                >
        ...
        {step === 2 && (
          <div className="flex flex-col gap-5">
                ...
                {CAMPAIGN_GOAL_OPTIONS.map((g) => (
                  ...
                  >
                    ...
                    {campaignGoal === g.value && (
                      <div className="w-3.5 h-3.5 bg-emerald-600 rounded-full flex items-center justify-center ml-0.5">
            ...
            <div className={`rounded-xl p-3 ${T.importBg} border ${T.importBorder}`}>
              <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider mb-1">Your Brand</p>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-[10px] text-emerald-500" style={{ fontFamily: font }}>{font}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="flex flex-col gap-5">
            <SectionTitle>Layout &amp; Output</SectionTitle>
            ...
            <Field label={`Flyer Variations: ${count}`}>
              <input
              ...
              <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                {[1,2,3,4,5,6,7,8].map((n) => (
                  <span key={n} className={count === n ? "text-emerald-600 font-bold" : ""}>{n}</span>
                ))}
              </div>
            </Field>
            ...
            {/* Full summary */}
            <div className={`rounded-xl p-3 ${T.importBg} border ${T.importBorder}`}>
              <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider mb-1">Summary</p>
              ...
              <p className="text-[10px] text-emerald-500 mt-1">
                ...
                {` · ${fileFormat} · ${SIZE_OPTIONS.find((s) => s.value === size)?.label} · ${orientation}`}
              </p>
            </div>
          </div>
        )}
