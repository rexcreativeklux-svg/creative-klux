...
const VideoAdsForm = ({
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/pexels?query=${encodeURIComponent(formData.brandName)}&per_page=8`,
        );
  ...
  const handleImportBrand = async () => {
    ...
    try {
      ...
      setFormData((p) => ({
        ...
        importedImages: normalizedImages.length
          ? normalizedImages
          : p.importedImages || [],
      }));
  ...
  return (
    <>
          ...
          {STEPS.map((s, idx) => {
            ...
            return (
              <React.Fragment key={s.id}>
                ...
                {idx < STEPS.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 rounded-full transition-all ${step > s.id ? "bg-blue-600" : "bg-gray-200"}`}
                  />
                )}
              </React.Fragment>
            );
      ...
      <div className="bg-surface rounded-lg py-5 px-2 flex flex-col gap-6">
            ...
            <Field label="Video Type">
              <div className="flex gap-2 flex-wrap">
                {VIDEO_TYPES.map((t) => (
                  <button
                    key={t.value}
                    onClick={() => field("videoType", t.value)}
                    className={`flex flex-col items-start px-5 py-2 rounded-lg border-2 cursor-pointer transition-all ${
                      formData.videoType === t.value
                        ? "border-blue-600 bg-blue-50"
                        : "border-gray-100 bg-gray-50 hover:border-gray-300"
                    }`}
                  >
                    <span
                      className={`text-xs font-semibold ${formData.videoType === t.value ? "text-blue-700" : "text-gray-700"}`}
                    >
                      {t.label}
                    </span>
              ...
              <div className="flex py-1 gap-2 flex-wrap">
                {VISUAL_STYLES.map((s) => (
                  <button
                    ...
                    className={`flex flex-col gap-2 px-3 py-1 rounded-md border cursor-pointer transition-all ${
                  ...
                  >
                    <span
                      className={`text-xs font-semibold ${formData.visualStyle === s.value ? "text-blue-700" : "text-gray-500"}`}
                    >
                      {s.label}
                    </span>
                  </button>
            ...
            <Field label="Audience">
              <div className="grid grid-cols-3 gap-2">
                {AUDIENCES.map((a) => (
                  ...
                  >
                    <p
                      className={`text-xs font-semibold ${formData.audience === a.value ? "text-blue-700" : "text-gray-700"}`}
                    >
                      {a.label}
                    </p>
        ...
        {step === 3 && (
          <div className="flex flex-col gap-4">
            ...
            <BrandImagesStrip
              images={
                formData.importedImages?.length
                  ? formData.importedImages
                  : undefined
              }
          ...
          {step < 3 ? (
            ...
            >
              {generating ? (
                ...
                <>
                  <Film className="w-4 h-4" /> Generate Video Ads
                </>
              )}
            </button>
          )}
      ...
      {generating && (
        <FullOverlayLoader
          title="Generating your videos"
          subtitle="Composing scenes, motion & sound"
        />
      )}
    </>
  );
