...
import { CheckCircle2, Send, Calendar, Download, Images, Scan, ChevronRight, ChevronDown, FileUp, FileSearch, FolderOpen, Video, Image, MoreVertical, ImagePlus, Loader2 } from 'lucide-react';
...
const ImageAdsCreatives = () => {
  ...
  const [searchResults, setSearchResults] = useState([
    { id: 1, src: "https://images.pexels.com/photos/316680/pexels-photo-316680.jpeg", alt: "Billboard advertising" },
    { id: 2, src: "https://images.pexels.com/photos/6476592/pexels-photo-6476592.jpeg", alt: "Digital ad" },
    { id: 3, src: "https://images.pexels.com/photos/6476591/pexels-photo-6476591.jpeg", alt: "Marketing image" },
    { id: 4, src: "https://images.pexels.com/photos/6476584/pexels-photo-6476584.jpeg", alt: "Ad campaign" },
    { id: 5, src: "https://images.pexels.com/photos/279739/pexels-photo-279739.jpeg", alt: "Shopping ad" },
    { id: 6, src: "https://images.pexels.com/photos/316680/pexels-photo-316680.jpeg", alt: "Billboard advertising" },
    { id: 7, src: "https://images.pexels.com/photos/6476592/pexels-photo-6476592.jpeg", alt: "Digital ad" },
    { id: 8, src: "https://images.pexels.com/photos/6476591/pexels-photo-6476591.jpeg", alt: "Marketing image" },
    { id: 9, src: "https://images.pexels.com/photos/6476584/pexels-photo-6476584.jpeg", alt: "Ad campaign" },
    { id: 10, src: "https://images.pexels.com/photos/279739/pexels-photo-279739.jpeg", alt: "Shopping ad" },
  ]);
  const [selectedImages, setSelectedImages] = useState([]);
  const [selectedMedia, setSelectedMedia] = useState([]);
  const [hashtagInput, setHashtagInput] = useState(""); // New state for hashtag input
  const fontOptions = [
  ...
  const handleFieldChange = useCallback((field, value) => {
    if (field === 'primaryColor' || field === 'secondaryColor') {
      ...
      setPostData((prev) => ({
        ...prev,
        [field]: Array.isArray(value) ? value.filter((tag) => tag.startsWith('#')).slice(0, 5) : [],
      }));
    } else {
  ...
  const handleHashtagKeyDown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      processHashtags();
      setHashtagInput(hashtags.join(' ')); // Update input to reflect processed hashtags
    }
  };
  ...
  const handleInspireMe = () => {
    setPostData((prev) => ({
      ...prev,
      description: "A vibrant short video showcasing our latest product launch!",
    }));
    setError("");
  };
...
const handleFileChange = async (e) => {
  ...
  const reader = new FileReader();
  reader.onload = () => {
    const newFile = new File([file], `image-${croppedImages.length}-${Date.now()}.png`, { type: file.type });
    setImageSrc((prev) => [...prev, reader.result]);
    setCroppedImages((prev) => [...prev, newFile]);
    setCreativeData((prev) => ({ ...prev, backgroundImage: newFile }));
  };
  reader.readAsDataURL(file);
};
  ...
  const saveCroppedImage = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      if (currentCropIndex < imageSrc.length - 1) {
        ...
        setCreativeData((prev) => ({ ...prev, backgroundImage: croppedImages[0] || file }));
      }
    }, 'image/png');
  };
  ...
  const handleSelectImage = async (src, source = 'brand') => {
    if (source === 'brand') {
      if (croppedImages.length < 5) {
        ...
        setCreativeData((prev) => ({ ...prev, backgroundImage: file }));
      }
    } else {
 ...
 const handleApplySelected = async () => {
  ...
  const newImages = magicMediaModalOpen ? selectedMedia : selectedImages;
  const newFiles = await Promise.all(
    newImages.map(async (src, index) => {
      const response = await fetch(src);
      const blob = await response.blob();
      return new File([blob], `image-${croppedImages.length + index}.png`, { type: blob.type });
    })
  );

  setImageSrc(newImages);
  setCroppedImages((prev) => [...prev, ...newFiles]);
  setShowCropper(true);
  ...
  const handleBulkPostNow = () => {
    if (!selectedAsset) {
      alert('Please select an asset to post.');
      return;
    }
    ...
    openAdsModal('post', assets);
    setSelectedAsset(null);
  };

  const handleBulkSchedule = () => {
    if (!selectedAsset) {
      alert('Please select an asset to schedule.');
      return;
    }
    const asset = result.assets.find(a => a.id === selectedAsset);
    const assets = asset ? [asset] : [];
    openAdsModal('schedule', assets);
    setSelectedAsset(null);
  };
  ...
  const handleBulkDownload = () => {
    ...
    const asset = result.assets.find(a => a.id === selectedAsset);
    if (asset) {
      setProcessedAssets((prev) => ({ ...prev, [selectedAsset]: { type: 'download', data: asset.src } }));
    }
    setSelectedAsset(null);
  };
  ...
  const handleContinue = () => {
    if (step === 1) {
      if (!postData.brandName || !postData.projectName) {
        setError("Please enter a brand name and project name.");
        return;
      }
    }
    if (step === 2) {
    ...
    if (step === 3 && croppedImages.length === 0) {
      setError("Please select or upload at least one background image.");
      return;
    }
    ...
    <div className='px-14'>
      ...
      {result ? (
        <div className="flex flex-col overflow-hidden w-full mt-3 justify-between gap-6 bg-white rounded-lg py-4">
          <div className="font-medium pb-4">Generated Ad Creatives</div>
                          ...
                          >
                            <div className="flex items-center justify-center mb-2">
                              <input
                                ...
                                className="w-4 h-4 cursor-pointer text-blue-600 border-gray-300 rounded focus:ring-blue-500 transition duration-300"
                                aria-label={`Select ${option.label}`}
                              />
                            </div>
              ...
              {step === 3 && (
                <div className='border border-gray-200 p-2 rounded-lg'>
                  <div className='flex border-b border-b-gray-200 pb-2 flex-row gap-2 mb-7 py-2'>
                    <div className='flex bg-gray-100 px-3 rounded-full justify-center items-center'>
                      <Images className='text-blue-700 w-6 h-6' />
                    </div>
                    <div className='flex flex-col justify-center'>
                      <h1 className='font-medium text-lg text-blue-700'>Choose Image</h1>
                      <p className='text-gray-600 text-xs'>Select or upload an image for your creative.</p>
                    </div>
                  </div>
                  ...
                  <div className='flex mb-10 flex-col space-y-4 border border-gray-200 py-5 bg-gray-50 justify-center items-center'>
                    ...
                    <div className='flex gap-4'>
                      ...
                      >
                        ...
                        <div className='mt-0.5'><Images className='w-4 h-4' /></div>
                      </button>
                      <input
                        type="file"
                        accept="image/*"
                    ...
                    <div className="fixed inset-0 bg-black/70 flex flex-col items-center justify-center z-50">
                      ...
                      <div className="flex gap-4 mt-4">
                        ...
                        <button
                          ...
                          className="px-4 cursor-pointer py-2 bg-gray-400 text-white rounded-lg hover:bg-gray-500"
                        >
                          Skip Crop
                        </button>
                        <button
                        ...
                        >
                          {currentCropIndex === imageSrc.length - 1 ? "Finish crop" : "Next"}
                        </button>
                      </div>
                    </div>
                  )}
              ...
              {step === steps.length && (
                <button
                  onClick={() => {
                    ...
                    const payload = {
                      postData,
                      images: croppedImages.map((img, index) => ({
                        index,
                    ...
                    setTimeout(() => {
                      ...
                      setResult({
                        assets: Array.from({ length: 8 }, (_, index) => ({
                          id: `asset_${index}`,
                          src: searchResults[index].src,
                          alt: searchResults[index].alt,
                        })),
                        type: null,
                        assetId: null,
                      });
