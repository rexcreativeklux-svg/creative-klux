...
import { X, Upload, Download, Loader2, Search, ChevronRight, Minus, Plus, HelpCircle, RefreshCw } from 'lucide-react';
...
function ColorSwatch({ color, selected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-8 h-8 rounded-full border-2 transition-all cursor-pointer hover:scale-110 ${selected ? 'border-blue-500 scale-110' : isWhite ? 'border-gray-300 hover:border-gray-400' : 'border-transparent hover:border-gray-300'}`}
...
export default function BgRemoverModal({ onClose, initialFile }) {
  ...
  // ── Core: local background removal via @imgly/background-removal ────────────
      ...
      >
        ...
        <div className="w-[64px] border-r border-gray-100 flex flex-col items-center py-2 bg-white flex-shrink-0">
          ...
          {/* <SidebarIcon active={activePanel === 'aitools'} onClick={() => setActivePanel('aitools')}
            icon={<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><path d="M10 2l1.5 5h5l-4 3 1.5 5-4-3-4 3 1.5-5.5-4-3h5z"/></svg>}
            label="AI tools" /> */}
        ...
        {/* ── Panel ── */}
        <div className="w-64 border-r border-gray-100 flex flex-col bg-white flex-shrink-0 overflow-y-auto">
          ...
          {activePanel === 'templates' && (
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                ...
                <button onClick={onClose} className="cursor-pointer p-1 rounded hover:bg-gray-100 transition-colors"><X className="w-4 h-4 text-gray-400" /></button>
              </div>
              <div className="px-3 py-2 border-b border-gray-100">
                <div className="flex gap-2 mb-2">
                  <button className="flex-1 py-1.5 text-xs font-medium rounded-full bg-gray-900 text-white cursor-pointer hover:bg-gray-700 transition-colors">From Photoroom</button>
                  <button className="flex-1 py-1.5 text-xs font-medium rounded-full border border-gray-200 text-gray-600 cursor-pointer hover:border-gray-400 hover:text-gray-800 transition-colors">Your Templates</button>
                </div>
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                  <input placeholder="Search templates" className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-gray-200 outline-none bg-gray-50 focus:border-blue-400 transition-colors cursor-text" />
                </div>
              </div>
              <div className="px-3 py-2">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-gray-700">Essentials</span>
                  <button className="text-xs text-gray-400 hover:text-blue-500 cursor-pointer transition-colors">See all</button>
                </div>
                <div className="grid grid-cols-2 gap-1.5 mb-4">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="aspect-square rounded-lg bg-gray-100 overflow-hidden border border-gray-200 hover:border-blue-400 cursor-pointer transition-all hover:shadow-md flex items-center justify-center">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {AI_BACKGROUNDS.slice(0,4).map((bg, i) => (
                    <div key={i} className="aspect-square rounded-lg overflow-hidden border border-gray-200 hover:border-blue-400 cursor-pointer transition-all hover:shadow-md relative group">
                      <img src={bg.img} alt={bg.name} className="w-full h-full object-cover" />
                      <div className="absolute top-1 right-1 w-5 h-5 bg-white/80 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 text-gray-600 text-xs transition-opacity">✦</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activePanel === 'resize' && (
            <div className="flex flex-col h-full">
              ...
              <div className="px-3 py-2 border-b border-gray-100">
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                  <input placeholder="Marketplaces, social media" className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-gray-200 outline-none bg-gray-50 focus:border-blue-400 transition-colors cursor-text" />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto px-3 py-2">
                ...
                {RESIZE_STANDARD.map(s => (
                  <button key={s.id} onClick={() => setSelectedResize(s.id)}
                    className={`w-full flex items-center gap-3 px-2 py-2.5 rounded-lg mb-0.5 transition-all cursor-pointer ${selectedResize === s.id ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50 border border-transparent hover:border-gray-200'}`}>
                ...
                {['Custom size', 'Original image'].map(name => (
                  <button key={name} className="w-full flex items-center gap-3 px-2 py-2.5 rounded-lg mb-0.5 hover:bg-gray-50 cursor-pointer transition-colors border border-transparent hover:border-gray-200">
                ...
                {RESIZE_SOCIAL.map(s => (
                  <button key={s.id} className="w-full flex items-center gap-3 px-2 py-2.5 rounded-lg mb-0.5 hover:bg-gray-50 cursor-pointer transition-colors border border-transparent hover:border-gray-200">
          ...
          {activePanel === 'position' && (
            <div className="flex flex-col h-full">
              ...
              <div className="px-3 py-3 border-b border-gray-100">
                <div className="flex gap-1 mb-4">
                  {['Original', 'Center', 'Custom'].map(t => (
                    <button key={t} onClick={() => setPosTab(t.toLowerCase())}
                      className={`flex-1 py-1.5 text-[11px] font-medium rounded-lg border transition-all cursor-pointer ${posTab === t.toLowerCase() ? 'border-gray-400 bg-white text-gray-800 shadow-sm' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200'}`}>
                      {t}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  {[['←', 'left'], ['→', 'right'], ['↑', 'top'], ['↓', 'bottom']].map(([arrow, key]) => (
                    <div key={key} className="flex items-center gap-1.5 border border-gray-200 rounded-lg px-2 py-1.5 focus-within:border-blue-400 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div>
                    ...
                    <select value={align} onChange={e => setAlign(e.target.value)}
                      className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 outline-none cursor-pointer hover:border-blue-400 transition-colors">
                ...
                <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer hover:text-gray-800 transition-colors">
                  <input type="checkbox" className="rounded cursor-pointer" /> Ignore padding on cropped sides
                </label>
              </div>
              <div className="px-3 py-3">
                ...
                <button className="mt-3 w-full py-2 rounded-xl border border-gray-200 text-xs font-semibold text-gray-500 flex items-center justify-center gap-1.5 hover:bg-gray-50 hover:border-blue-400 hover:text-blue-600 cursor-pointer transition-all">
              ...
              <div className="flex border-b border-gray-100">
                {[
                  ...
                  <button key={id} onClick={() => setBgTab(id)}
                    className={`flex-1 flex flex-col items-center gap-1 py-2 text-[11px] font-medium transition-colors border-b-2 cursor-pointer ${bgTab === id ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                    {icon}{name}
                  </button>
                ))}
              </div>
                ...
                {bgTab === 'color' && (
                  <>
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <label className="w-8 h-8 rounded-full cursor-pointer border-2 border-gray-200 overflow-hidden flex items-center justify-center hover:border-blue-400 transition-colors"
                        title="Custom color"
                      ...
                      <button className="w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-gray-500 hover:border-blue-400 hover:text-blue-500 cursor-pointer transition-colors" title="Eyedropper">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {AI_BACKGROUNDS.map(bg => (
                      <button key={bg.id} onClick={() => applyAiBackground(bg)}
                        className={`relative rounded-xl overflow-hidden aspect-video border-2 transition-all cursor-pointer hover:shadow-md ${selectedAiBg === bg.id ? 'border-blue-500' : 'border-transparent hover:border-gray-300'}`}>
                        <img src={bg.img} alt={bg.name} className="w-full h-full object-cover" />
                ...
                {bgTab === 'ai' && (
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe your background (e.g. soft studio gradient, outdoor garden…)"
                      className="w-full text-xs border border-gray-200 rounded-xl p-2.5 outline-none resize-none focus:border-blue-400 bg-gray-50 cursor-text transition-colors"
                      rows={3}
                    />
                    <button
                      onClick={() => toast.info('Type a description and generate!')}
                      className="mt-2 w-full py-2 rounded-xl text-xs font-semibold text-white cursor-pointer hover:opacity-90 active:scale-95 transition-all"
                  ...
                  {SHADOW_PRESETS.map(s => (
                    <button key={s.id} onClick={() => setSelectedShadow(s.id)}
                      ...
                      <div className="w-14 h-14 bg-gray-50 rounded-xl flex items-center justify-center relative">
                        ...
                        {s.id === 'none' && <div className="w-8 h-8 bg-white border-2 border-blue-300 rounded-lg" />}
                        {s.id !== 'none' && <div className="absolute top-1 right-1 w-4 h-4 bg-blue-100 rounded flex items-center justify-center"><svg viewBox="0 0 8 8" fill="currentColor" className="w-2.5 h-2.5 text-blue-500"><path d="M4 1l.5 1.5h2L5 3.5l.5 2L4 5 2.5 5.5 3 3.5 1.5 2.5h2z"/></svg></div>}
                      </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {AI_TOOLS.map(tool => (
                  <button key={tool.id}
                    ...
                    className="w-full flex items-center gap-3 px-3 py-3 border-b border-gray-50 hover:bg-gray-50 hover:border-l-2 hover:border-l-blue-400 transition-all cursor-pointer">
        ...
        <div className="flex-1 flex flex-col bg-[#e8e8e8] relative overflow-hidden">
          <button onClick={onClose} className="absolute top-3 right-3 z-20 w-8 h-8 bg-white rounded-full border border-gray-200 flex items-center justify-center hover:bg-red-50 hover:border-red-300 hover:text-red-500 shadow-sm cursor-pointer transition-all">
            <X className="w-4 h-4 text-gray-600" />
          </button>

          <div className="absolute top-3.5 left-4 z-20 text-xs text-gray-500">
            {removing ? 'Processing…' : removedUrl ? 'Background removed ✓' : 'Upload an image'}
          </div>
              ...
              <div className="flex flex-col items-center gap-4 w-56">
                ...
                <div className="text-center">
                  ...
                  {removingProgress > 0 ? (
                    ...
                    <p className="text-gray-400 text-xs mt-1">Loading model…</p>
                  )}
                </div>
              </div>
            ) : displayUrl ? (
              <div className="relative">
                <div className="rounded-2xl overflow-hidden shadow-2xl relative"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6">
                    {applyingAiBg ? (
                      ...
                      <img
                        src={aiResultUrl && !showBefore ? aiResultUrl : (removedUrl && !showBefore ? removedUrl : originalUrl)}
                        alt="product"
                        className="max-w-full max-h-full object-contain"
                        style={{ ...shadowStyle, transform: `scale(${zoom / 100})`, transformOrigin: 'center', transition: 'transform 0.2s' }}
                      />
                    )}
                  </div>
                </div>
                ...
                {originalUrl && removedUrl && (
                  <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 flex bg-white rounded-full shadow border border-gray-200 overflow-hidden text-xs font-medium">
                    <button onClick={() => setShowBefore(false)} className={`px-4 py-1.5 transition-colors cursor-pointer ${!showBefore ? 'bg-gray-800 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>After</button>
                    <button onClick={() => setShowBefore(true)} className={`px-4 py-1.5 transition-colors cursor-pointer ${showBefore ? 'bg-gray-800 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>Before</button>
                  </div>
                )}
              </div>
            ) : (
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-white/80 backdrop-blur-sm border-t border-gray-100">
            ...
            <div className="flex items-center gap-1 bg-white rounded-full border border-gray-200 shadow-sm px-2 py-1">
              <button onClick={() => setZoom(z => Math.max(25, z - 25))} className="p-1 hover:bg-gray-100 rounded-full text-gray-500 cursor-pointer transition-colors"><Minus className="w-3 h-3" /></button>
              <span className="text-xs text-gray-500 w-10 text-center">{zoom}%</span>
              <button onClick={() => setZoom(z => Math.min(200, z + 25))} className="p-1 hover:bg-gray-100 rounded-full text-gray-500 cursor-pointer transition-colors"><Plus className="w-3 h-3" /></button>
