...
- **Where the refresh token lives:** `setStoredXRefresh/getStoredXRefresh` keep it in **localStorage** keyed by integration id (`ck_x_refresh_<id>`), and it's also sent to the backend as **`int_refresh_token`** in `saveIntegration`. **(2026-06-23) Backend dev confirms the integrations endpoint now stores/returns credentials**, so connect-time storage + read-back work and `publishToTwitter` reads `integration.int_refresh_token` first (localStorage fallback). **⚠️ Remaining gap — rotation write-back:** the token rotates every use and the NEW one is written back **only to localStorage** ([:1252](src/(lib)/integration.js#L1252), [:1780](src/(lib)/integration.js#L1780)), never to the backend — so the backend's copy is valid exactly once; after the first post it's stale, and another device fails. Closing it needs the lib publish/fetch functions to **return** the rotated token to their React caller (a plain lib can't call `saveIntegration`), plus an integration **update path** (no `updateIntegration`/PUT exists yet — only POST/GET/DELETE; depends on whether POST upserts-merge or a new PUT is added). **Meta/IG/LinkedIn are NOT affected** — their tokens are long-lived and non-rotating.
