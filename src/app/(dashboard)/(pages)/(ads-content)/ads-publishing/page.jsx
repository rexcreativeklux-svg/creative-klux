...
//   TrendingUp, AlertCircle, Send, Edit2, X, Search, ChevronLeft, ChevronRight, Filter
// } from 'lucide-react';
...
//   useEffect(() => { setPage(1); }, [statusFilter, platformFilter, search]);
...
//         const r = await publishToFacebook({ access_token: account.access_token, page_id: account.page_id, image_url: post.image_url, caption: post.caption });
//         postId = r.post_id;
//       } else if (post.platform === 'instagram') {
//         const r = await publishToInstagram({ access_token: account.access_token, ig_user_id: account.ig_user_id, image_url: post.image_url, caption: post.caption });
//         postId = r.post_id;
//       } else if (post.platform === 'meta_ads') {
//         const r = await publishToMetaAds({ access_token: account.access_token, ad_account_id: account.ad_account_id, image_url: post.image_url, caption: post.caption, campaign_name: post.project_title });
//         postId = r.post_id;
//       } else {
...
//         if (idx >= 0) {
//           all[idx].stats = { ...all[idx].stats, ...newStats, last_updated: new Date().toISOString() };
//           localStorage.setItem('creativeklux_published_posts', JSON.stringify(all));
//           reload();
//           toast.success('Stats refreshed from platform');
//         }
//       } else {
//         toast.info('Real-time stats are only available for connected Facebook/Instagram posts.');
//       }
//     } catch (err) {
...
//         <div>
//           <h1 className="text-2xl font-bold text-gray-900 tracking-tight mb-1">{typeLabel} Publishing</h1>
//           <p className="text-gray-500 text-sm">Track your {typeLabel.toLowerCase()} published and scheduled creatives.</p>
//         </div>
...
//             >
//               <RefreshCw className={`w-3.5 h-3.5 mr-2 text-gray-500 ${fetchingLive ? 'animate-spin' : ''}`} />
...
//           <SummaryCard label="Total Ads"         value={posts.length}                                                                Icon={BarChart3}    />
...
//           </div>
//           <p className="text-gray-800 font-medium mb-1">No ads match your filters</p>
//           <p className="text-gray-400 text-sm mb-4">Try adjusting the platform, status, or search query.</p>
//           <button
...
//             onClick={() => { setStatusFilter('all'); setPlatformFilter('all'); setSearch(''); }}
//           >
//             Clear Filters
//           </button>
//         </div>
...
//           </div>
//           <p className="text-gray-900 font-semibold mb-1">No published or scheduled ads yet</p>
...
//                           {post.image_url
//                             ? <img src={post.image_url} alt="" className="w-9 h-9 rounded-lg object-cover flex-shrink-0 border border-gray-200" />
//                             : <div className="w-9 h-9 rounded-lg bg-gray-100 flex-shrink-0 border border-gray-200" />
//                           }
...
//                       <td className="px-3 py-3 text-center"><StatCell value={stats.likes} /></td>
//                       <td className="px-3 py-3 text-center"><StatCell value={stats.shares} /></td>
...
//               Showing {filteredPosts.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filteredPosts.length)} of {filteredPosts.length} ads
//             </span>
//             {totalPages > 1 && (
...
//                     style={n === page ? { backgroundColor: BRAND, color: '#fff' } : { color: '#6b7280' }}
//                     onMouseEnter={e => { if (n !== page) e.currentTarget.style.backgroundColor = '#f3f4f6'; }}
    ...
    <div className="">
      ...
      {filteredPosts.length === 0 && posts.length > 0 && (
        <div className="rounded-xl border border-dashed border-gray-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="text-gray-800 font-medium mb-1">No posts match your filters</p>
          <p className="text-gray-400 text-sm mb-4">Try adjusting the platform, status, or search query.</p>
          <button
            className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors"
            onClick={() => { setStatusFilter('all'); setPlatformFilter('all'); setSearch(''); }}
