...
- **Publish (My Creations → `PublishModal`) is half-real.** The Publish button on a creative opens `src/app/(components)/PublishModal.jsx`. It loads connected accounts via `fetchIntegrations()`, pre-fills a caption from the creative's copy, and posts **directly browser → `graph.facebook.com`** (not via the Laravel backend) using the integration's stored `int_token`/`int_id`. **Only Facebook and Instagram actually publish** (`publishToFacebook`/`publishToInstagram` in `src/(lib)/integration.js`, real Graph API calls). **Every other platform (X, LinkedIn, YouTube, TikTok, Pinterest, all `*_ads`) is a simulated stub** — `await sleep(1200)` then a fake "Published!" toast. The image is sent as `creative.image` (a public https URL — blob/local URLs won't work).
...
- **Photo Editor rebuilt Photoroom-style** — base image moved to a box model with **8-direction drag-resize** (corners lock aspect, edges stretch) on base image + every layer; new **Layers panel** (show/hide/reorder/duplicate/delete); Brand Kit / Add text / Templates / Backgrounds / Resize panels all rebuilt (searchable, grouped, brand-kit palettes, Pexels search, real target sizes via `exportSize`, circular Profile Pics via `canvasRound`). See "Product Photos — Photo Editor". ⚠️ Built + compiles; **not yet click-tested or deployed** (production `app.creativeklux.com` still runs older code).
- **Dark mode** (whole app) — next-themes + palette-variable override in `globals.css`; light untouched. See "Theming / dark mode".
...
- **Product Photos → Background Remover + Batch** — `BackgroundRemoverModal` (core removal via `@imgly` Web Worker pool, `isnet_fp16`); `/product-photos/batch` opens selected images in batch mode. Image-background library = 8 categories / 47 verified Pexels IDs (now also reused by the editor's Backgrounds panel).
