...
const ImagePropertiesModal = ({ image, onClose, onApply, existingData }) => {
  ...
  const [objectFit, setObjectFit] = useState(
    existingData?.objectFit || "cover",
  );
  const [borderRadius, setBorderRadius] = useState(
    existingData?.borderRadius ?? 8,
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
      <div className="bg-surface dark:bg-dark-card rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border border-light-border dark:border-dark-border flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="px-6 py-4 border-b border-light-border dark:border-dark-border flex items-center justify-between bg-linear-to-r from-primary-700 to-primary-600 text-white shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-surface/20 rounded-lg transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <div className="p-5 flex flex-col gap-5 bg-surface dark:bg-dark-card">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-900 dark:text-dark-bold mb-1.5">
                  Caption{" "}
                  <span className="font-normal text-light-text dark:text-dark-text">
                    (optional)
                  </span>
                </label>
                <input
              ...
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <div className="flex-1 relative">
                    ...
                    />
                    <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-light-text dark:text-dark-text pointer-events-none">
                      W
                    </span>
                  </div>
                  <span className="text-light-text dark:text-dark-text font-bold text-sm select-none">
                    ×
                  </span>
                  <div className="flex-1 relative">
                    <input
                      ...
                      <p
                        className={`text-xs font-semibold ${
                          objectFit === value
                            ...
                            : "text-gray-900 dark:text-dark-bold"
                        }`}
                      >
                        {label}
                      </p>
                      <p className="text-xs text-light-text dark:text-dark-text mt-0.5">
                        {desc}
                      </p>
                    </button>
                  ))}
              ...
              <div className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border">
                <div>
                  <p className="text-xs font-semibold text-gray-900 dark:text-dark-bold">
                    Lazy Load
                  </p>
                ...
                >
                  <div
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-surface rounded-full shadow transition-transform ${
                      lazy ? "translate-x-5" : "translate-x-0"
                    }`}
                  />
                </button>
              </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-light-border dark:border-dark-border flex items-center justify-between gap-3 shrink-0 bg-surface dark:bg-dark-card">
          <button
            onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      <div className="bg-surface dark:bg-dark-card rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden border border-light-border dark:border-dark-border flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs opacity-75 mt-0.5">
                Powered by AI — processed on our server
              </p>
            </div>
          </div>
          <button
        ...
        {/* Warning banner */}
        {isProcessing && (
          <div className="px-6 py-2.5 bg-light-bg dark:bg-dark-bg border-b border-light-border dark:border-dark-border flex items-center gap-2 shrink-0">
            <Clock className="w-4 h-4 text-primary-600 shrink-0" />
            <p className="text-xs text-light-text dark:text-dark-text">
              AI background removal takes{" "}
              <strong className="text-dark-bold dark:text-dark-bold">
                5–15 seconds
              </strong>
              . Please wait.
            </p>
        ...
        <div className="p-6 bg-surface dark:bg-dark-card">
          <div className="grid grid-cols-2 gap-5">
            {/* Original */}
            <div>
              <p className="text-xs font-semibold text-light-text dark:text-dark-text uppercase tracking-wider mb-2">
                Original
              </p>
                ...
                {isProcessing && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-surface/90 dark:bg-dark-card/90 p-5">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-semibold text-dark-bold dark:text-dark-bold">
                        {PROCESSING_STEPS[stepIdx].label}
                      </p>
                ...
                {isError && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-5 gap-3">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-accent-pink mt-1 max-w-[180px]">
                        {error}
                      </p>
                    </div>
                    <button
        ...
        {/* Footer */}
        <div className="px-6 pb-5 flex items-center justify-between gap-3 shrink-0 bg-surface dark:bg-dark-card">
          <div>
            {(isDone || isError) && (
              <button
                onClick={handleProcess}
...
const CropModal = ({ image, onClose, onApply }) => {
  ...
  const getHandles = (box) => ({
    ...
    se: {
      x: box.x + box.w - HANDLE_SIZE / 2,
      y: box.y + box.h - HANDLE_SIZE / 2,
    },
  });
  ...
  const draw = useCallback(() => {
    ...
    Object.values(getHandles(cropBox)).forEach((h) =>
  ...
  const hitHandle = (pos) => {
    for (const [key, h] of Object.entries(getHandles(cropBox))) {
      if (
        pos.x >= h.x &&
        pos.x <= h.x + HANDLE_SIZE &&
        pos.y >= h.y &&
        pos.y <= h.y + HANDLE_SIZE
      )
        return key;
    }
    return null;
  ...
  const handleApply = async () => {
    ...
    out.width = cropBox.w * scaleX;
    out.height = cropBox.h * scaleY;
    out
      .getContext("2d")
      .drawImage(
        img,
    ...
    out.toBlob(
      async (blob) => {
        try {
          ...
          const savedUrl =
            res.data?.image ?? res.data?.image_url ?? res.data?.url;
          onApply({ url: savedUrl, image_url: savedUrl });
        } catch {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60]">
      <div className="bg-surface dark:bg-dark-card rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border border-light-border dark:border-dark-border flex flex-col max-h-[90vh]">
        ...
        {/* Toolbar */}
        <div className="px-6 py-3 border-b border-light-border dark:border-dark-border flex items-center gap-4 shrink-0 bg-light-bg dark:bg-dark-bg">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-light-text dark:text-dark-text uppercase tracking-wider">
              Aspect:
            </span>
            <div className="flex gap-1">
              {aspects.map((a) => (
            ...
            <img
              ...
              onLoad={(e) => {
                ...
                const scale = Math.min(
                  maxW / e.target.naturalWidth,
                  maxH / e.target.naturalHeight,
                  1,
                );
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-light-border dark:border-dark-border flex justify-between items-center shrink-0 bg-surface dark:bg-dark-card">
          <p className="text-xs text-light-text dark:text-dark-text">
...
const ImageCard = ({
  ...
  return (
    <div className="group relative bg-surface dark:bg-dark-card rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-light-border dark:border-dark-border break-inside-avoid mb-4">
      ...
      <div className="absolute inset-0 bg-linear-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            ...
            {!image.isVideo && (
              <>
                ...
                <button
                  ...
                  className="w-full px-3 py-2 text-left text-sm flex items-center gap-2.5 text-light-text dark:text-dark-text hover:bg-light-bg dark:hover:bg-dark-bg hover:text-dark-bold dark:hover:text-dark-bold transition-colors"
                >
                  <Wand2 className="w-4 h-4 text-primary-400" /> Remove
                  Background
                </button>
              </>
            )}
...
const ImageUploadModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      fetchMyImages();
      handleSearch("high resolution landscape");
    }
    ...
    async (query = searchQuery) => {
      ...
      try {
        ...
        if (orientation !== "All Orientations")
          params.append("orientation", pexelsOrientationMap[orientation]);
        const endpoint =
          mediaType === "Photos"
            ? `https://api.pexels.com/v1/search?${params}`
            : `https://api.pexels.com/videos/search?${params}`;
        const response = await fetch(
          `/api/pexels?endpoint=${encodeURIComponent(endpoint)}`,
        );
  ...
  const handleFile = async (file) => {
    ...
    if (!file.type.startsWith("image/")) {
      alert("Please upload images only (JPG, PNG, GIF)");
      return;
    }
  ...
  useEffect(() => {
    const handler = (e) => {
      if (mediaRef.current && !mediaRef.current.contains(e.target))
        setMediaOpen(false);
      if (orientationRef.current && !orientationRef.current.contains(e.target))
    ...
    <>
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4 py-2">
        <div className="bg-surface dark:bg-dark-card rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] overflow-hidden border border-light-border dark:border-dark-border flex flex-col">
          {/* Header */}
          <div className="px-6 py-5 border-b border-light-border dark:border-dark-border flex items-center justify-between bg-linear-to-r from-primary-700 to-primary-600 text-white shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm opacity-90 mt-0.5">
                  Click any image to configure and insert
                </p>
              </div>
            </div>
            <button
          ...
          {/* Search bar */}
          <div className="px-6 py-4 border-b border-light-border dark:border-dark-border bg-light-bg dark:bg-dark-bg shrink-0">
            <div className="flex gap-3">
              <div className="flex-1 relative">
                {isUrl(searchQuery) ? (
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <div className="relative" ref={orientationRef}>
                ...
                >
                  {orientation}{" "}
                  <ChevronDown
                    className={`w-4 h-4 transition-transform ${orientationOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {orientationOpen && (
          ...
          {/* Tabs */}
          <div className="border-b border-light-border dark:border-dark-border bg-surface dark:bg-dark-card shrink-0">
            <div className="px-6 flex gap-6">
              {[
                ...
                <button
                  ...
                  className={`py-3 border-b-2 transition-all font-medium text-sm ${
                    activeTab === tab.id
              ...
              (isSearching ? (
                ...
                <div className="flex items-center justify-center min-h-[400px]">
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-dark-bold dark:text-dark-bold mb-2">
                      Start Your Search
                    </h3>
                    <p className="text-light-text dark:text-dark-text">
                      Search photos or paste an image URL
                    </p>
                  </div>
                </div>
              ))}
