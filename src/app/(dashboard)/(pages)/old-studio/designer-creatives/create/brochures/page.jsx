...
const BrochureCreationPage = () => {
  ...
  useEffect(() => {
    if (activeBrand) {
      setPostData(prev => ({
        ...
        projectName: activeBrand.name || prev.brandName,
        ...
        hashtags: prev.hashtags.length > 0 ? prev.hashtags : ['#Brochure', '#Marketing', '#Business'],
      }));
  ...
  const handleFoldTypeChange = (e) => {
    setPostData((prev) => ({ ...prev, foldType: e.target.value }));
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/pexels?query=${encodeURIComponent(`${postData.brandName} ${postData.campaignGoal} brochure`)}&per_page=20}`);
        const data = await res.json();
        setRecommendedImages((data.photos || []).map(p => ({
          id: p.id,
          src: p.src.medium,
          large: p.src.large2x,
          alt: p.alt || 'Recommended brochure image',
        })));
      ...
      {result ? (
        ...
        <div className="flex flex-row gap-10 w-full">
          ...
          <div className="flex flex-col w-full mt-5 justify-between gap-10 bg-surface rounded-2xl p-4">
            ...
            {/* Step 2: Goals & Formatting — UNCHANGED */}
            {step === 2 && (
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {foldTypeOptions.map((option) => (
                        <div
                          ...
                          className={`cursor-pointer flex flex-row gap-2 border rounded-lg p-2 text-center text-xs ${postData.foldType === option.value ? 'border-blue-700 bg-blue-50' : 'border-gray-200'}`}
                          aria-label={`Select ${option.label}`}
                        >
                          <div className="flex justify-center">
                            <input
                              type="checkbox"
                              checked={postData.foldType === option.value}
            ...
            <div className="flex justify-between items-center pt-6 ">
              ...
              <button onClick={() => step === 3 ? handleGenerate() : setStep(step + 1)}
                ...
                {loading.generate ? <Loader2 className="w-5 h-5 animate-spin" /> : (step === 3 ? 'Generate Brochures' : 'Continue')}
              </button>
            </div>
          </div>
        </div>
